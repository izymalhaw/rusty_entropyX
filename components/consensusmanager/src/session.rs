...
use entropyx_utils::sync::rwlock::*;
...
impl ConsensusSessionOwned {
    ...
    /// ENX-CHANGE-BURN: Get the total burned amount
    pub fn get_total_burned_amount(&self) -> ConsensusResult<Option<u64>> {
        self.consensus.get_total_burned_amount()
    }

    /// ENX-CHANGE-BURN: Get the burn record by block hash
    pub fn get_burn_record(&self, block_hash: Hash) -> ConsensusResult<Option<Arc<BurnRecord>>> {
        self.consensus.get_burn_record(block_hash)
    }

    /// ENX-CHANGE-BURN: Get the burn records by range
    pub fn get_burn_records_by_range(&self, start_daa: u64, end_daa: u64) -> ConsensusResult<Vec<Arc<BurnRecord>>> {
        self.consensus.get_burn_records_by_range(start_daa, end_daa)
    }

    /// ENX-CHANGE-BURN: Get the current burn fee
    pub fn get_current_burn_fee(&self, current_daa_score: u64) -> ConsensusResult<u64> {
        self.consensus.get_current_burn_fee(current_daa_score)
    }
