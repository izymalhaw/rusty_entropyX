...
use crate::pb::{entropy_x_message::Payload, ReadyMessage, VerackMessage, VersionMessage};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use crate::{IncomingRoute, EntropyXMessagePayloadType, Router};
use entropyx_core::debug;

/// Implements the EntropyX peer-to-peer handshake protocol
pub struct EntropyXHandshake<'a> {
    router: &'a Router,
...
impl<'a> EntropyXHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![EntropyXMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![EntropyXMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![EntropyXMessagePayloadType::Ready]),
        }
    }
