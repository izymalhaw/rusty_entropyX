...
use entropyx_connectionmanager::ConnectionManager;
use entropyx_consensus_core::api::{BlockValidationFuture, BlockValidationFutures};
use entropyx_consensus_core::block::Block;
use entropyx_consensus_core::config::Config;
use entropyx_consensus_core::errors::block::RuleError;
use entropyx_consensus_core::tx::{Transaction, TransactionId};
use entropyx_consensus_notify::{
    notification::{Notification, PruningPointUtxoSetOverrideNotification},
    root::ConsensusNotificationRoot,
};
use entropyx_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy};
use entropyx_core::{
    debug, info,
    entropyx_env::{name, version},
    task::tick::TickService,
};
use entropyx_core::{time::unix_now, warn};
use entropyx_hashes::Hash;
use entropyx_mining::mempool::tx::{Orphan, Priority};
use entropyx_mining::{manager::MiningManagerProxy, mempool::tx::RbfPolicy};
use entropyx_notify::notifier::Notify;
use entropyx_p2p_lib::{
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{entropy_x_message::Payload, InvRelayBlockMessage},
    ConnectionInitializer, Hub, EntropyXHandshake, PeerKey, PeerProperties, Router,
};
use entropyx_utils::iter::IterExtensions;
...
impl FlowContext {
    ...
    ///
    /// _GO-ENTROPYX: OnNewBlock + broadcastTransactionsAfterBlockAdded_
