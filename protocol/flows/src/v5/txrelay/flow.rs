...
use entropyx_consensusmanager::ConsensusProxy;
...
use entropyx_p2p_lib::{
    ...
    pb::{entropy_x_message::Payload, RequestTransactionsMessage, TransactionNotFoundMessage},
...
impl RelayTransactionsFlow {
    ...
    pub fn invs_channel_size() -> usize {
        ...
        // Note: in go-entropyx we have 10,000 for this channel combined with tx channel.
        4096
    }
            ...
            match res {
                ...
                | Err(MiningManagerError::MempoolError(RuleError::RejectNonStandard(..))) => {
                    ...
                    if self.spam_counter % 100 == 0 {
                        entropyx_core::warn!("Peer {} has shared {} spam/non-standard txs ({:?})", self.router, self.spam_counter, res);
                    }
                }
                Err(_) => {}
            }
