...
use entropyx_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
...
use entropyx_p2p_lib::{
    ...
    pb::{entropy_x_message::Payload, InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage},
    IncomingRoute, Router, SharedIncomingRoute,
};
use entropyx_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-entropyx and this codebase) we could query only the last one,
