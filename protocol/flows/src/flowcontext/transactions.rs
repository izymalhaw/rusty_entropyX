...
use entropyx_p2p_lib::{
    make_message,
    pb::{entropy_x_message::Payload, InvTransactionsMessage, EntropyXMessage},
    Hub,
};
...
const REBROADCAST_FREQUENXY: u64 = 3;
...
impl TransactionsSpread {
    ...
    pub fn should_rebroadcast(&self) -> bool {
        self.scanning_job_count % REBROADCAST_FREQUENXY == 0
    }
    ...
    /// capacity.
    ///
    /// _GO-ENTROPYX: EnqueueTransactionIDsForPropagation_
    ...
    async fn broadcast(&self, msg: EntropyXMessage, should_throttle: bool) {
        if should_throttle {
