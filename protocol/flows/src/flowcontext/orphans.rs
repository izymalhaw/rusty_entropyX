...
use entropyx_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
use entropyx_core::debug;
use entropyx_hashes::Hash;
use entropyx_utils::option::OptionExtensions;
use rand::Rng;
use std::{
...
mod tests {
    ...
    use entropyx_consensusmanager::{ConsensusInstance, SessionLock};
    use entropyx_core::assert_match;
    ...
    impl ConsensusApi for MockProcessor {
        ...
        fn get_total_burned_amount(&self) -> Result<Option<u64>, ConsensusError> {
            Ok(Some(0))
        }

        fn get_burn_records_by_range(&self, _start: u64, _end: u64) -> Result<Vec<Arc<BurnRecord>>, ConsensusError> {
            Ok(vec![])
        }

        fn get_burn_record(&self, _hash: Hash) -> Result<Option<Arc<BurnRecord>>, ConsensusError> {
            Ok(None)
        }

        fn get_current_burn_fee(&self, _daa_score: u64) -> Result<u64, ConsensusError> {
            Ok(0)
        }
    }
