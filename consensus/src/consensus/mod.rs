...
use crate::{
    ...
    model::{
        ...
        stores::{
            ...
            burned_fees::BurnedFeesStoreReader, // ENX-CHANGE-BURN:
            DB,
        },
    },
    pipeline::{
...
use entropyx_consensus_core::{
    ...
    burn::BurnRecord, // ENX-CHANGE-BURN:
    coinbase::MinerData, 
...
use entropyx_consensusmanager::{SessionLock, SessionReadGuard};

use entropyx_database::prelude::StoreResultExtensions;
...
impl ConsensusApi for Consensus {
    ...
    fn get_current_block_color(&self, hash: Hash) -> Option<bool> {
        ...
        while let Some(Reverse(SortableBlock { hash: decedent, .. })) = heap.pop() {
            if self.services.reachability_service.is_chain_ancestor_of(decedent, sink) {
                ...
                // unreachable.
                entropyx_core::warn!("DAG topology inconsistency: {decedent} is expected to be a merging block of {hash}");
    ...
    fn get_chain_block_samples(&self) -> Vec<DaaScoreTimestamp> {
        ...
        if self.config.net.network_type == NetworkType::Mainnet {
            ...
            // Source: https://github.com/entropyxgang/entropyx-py-explorer/blob/main/src/tx_timestamp_estimation.ipynb
            // For context see also: https://github.com/entropyxgang/entropyx-py-explorer/blob/main/src/genesis_proof.ipynb
    ...
    fn get_total_burned_amount(&self) -> ConsensusResult<Option<u64>> {
        match self.storage.burned_fees_store.get_latest_burn_record() {
            Ok(Some(item)) => Ok(Some(item.total_burned)),
            Ok(None) => Ok(Some(0)),
            Err(_) => Err(ConsensusError::General("Get latest burn record error")),
        }
    }

    /// ENX-CHANGE-BURN: 
    /// Get the burn records by range
    fn get_burn_records_by_range(&self, start_daa: u64, end_daa: u64) -> ConsensusResult<Vec<Arc<BurnRecord>>> {
        self.storage.burned_fees_store.get_burn_records_by_range(start_daa, end_daa)
        .map_err(|_| ConsensusError::General("Get burn records by range error"))
    }

    /// ENX-CHANGE-BURN: 
    /// Get the burn record by block hash
    fn get_burn_record(&self, block_hash: Hash) -> ConsensusResult<Option<Arc<BurnRecord>>> {
        self.storage.burned_fees_store.get_burn_record(block_hash)
        .map_err(|_| ConsensusError::General("Get burn record error"))
    }

    /// ENX-CHANGE-BURN: 
    /// Get the current burn fee
    fn get_current_burn_fee(&self, current_daa_score: u64) -> ConsensusResult<u64> {
        Ok(self.virtual_processor.burn_fee_calculator.get_current_burn_fee(current_daa_score))
    }
}
