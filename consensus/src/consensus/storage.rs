use crate::{
    ...
    model::stores::{
        ...
        burned_fees::DbBurnedFeesStore, // ENX-CHANGE-BURN:
        DB,
    },
...
use entropyx_consensus_core::{blockstatus::BlockStatus, BlockHashSet};
...
pub struct ConsensusStorage {
    ...
    /// ENX-CHANGE-BURN: 
    /// The burned fees store
    pub burned_fees_store: Arc<DbBurnedFeesStore>,
}
...
impl ConsensusStorage {
    pub fn new(db: Arc<DB>, config: Arc<Config>) -> Arc<Self> {
        ...
        // ENX-CHANGE-BURN: Initialize the burned fees store
        let burn_records_builder = PolicyBuilder::new()
            .bytes_budget(scaled(10_000_000)) // 10MB
            .tracked_bytes();
        let daa_index_builder = PolicyBuilder::new()
            .bytes_budget(scaled(10_000_000)) // 10MB
            .tracked_bytes();

        let burned_fees_store = Arc::new(DbBurnedFeesStore::new(
            db.clone(), 
            burn_records_builder.build(), 
            daa_index_builder.build()
        ));
        burned_fees_store.init().unwrap();
        ...
        Arc::new(Self {
            ...
            lkg_virtual_state,
            burned_fees_store, // ENX-CHANGE-BURN:
        })
    }
}
