...
impl CoinbaseManager {
    pub fn new(
        ...
        // In a 10 BPS network, the induced increase in total rewards is 51 ENX (see tests::calc_high_bps_total_rewards_delta())
    ...
    /// ENX-CHANGE: Block subsidy calculation function that implements halving-based emission schedule
    /// 
    /// This function calculates the block subsidy (mining reward) based on the following rules:
    /// - Initial subsidy starts at pre_deflationary_phase_base_subsidy
    /// - Subsidy halves every deflationary_phase_daa_score blocks (halving period)
    /// - After 64 halvings, subsidy becomes 0 to ensure finite supply
    /// - Total supply is capped at 21 billion ENX (2.1e16 sompi)
    /// 
    /// Parameters:
    /// - daa_score: The DAA score of the block to calculate subsidy for
    /// 
    /// Returns:
    /// - The calculated block subsidy in sompi
    pub fn calc_block_subsidy(&self, daa_score: u64) -> u64 {
        // ENX-CHANGE: Calculate which halving period the current block is in
        let current_period = daa_score / self.deflationary_phase_daa_score;
        
        // ENX-CHANGE: If we've gone through 64 halving periods, subsidy becomes 0
        // This ensures a finite total supply
        if current_period >= 64 {
            return 0;
        }
        
        // Calculate current subsidy by right-shifting (dividing by 2) for each period
        // e.g. period 0: full subsidy
        //      period 1: half subsidy
        //      period 2: quarter subsidy, etc.
        let current_subsidy = self.pre_deflationary_phase_base_subsidy >> current_period;
        
        // Calculate total coins issued so far
        let total_supply = self.calculate_total_supply(daa_score);
        
        // Cap total supply at 21 billion ENX (in sompi units)
        // Return 0 if we've hit the cap, otherwise return calculated subsidy
        if total_supply >= MAX_SOMPI {
            0
        } else {
            current_subsidy
        }
    ...
    /// ENX-CHANGE: Helper function to calculate total coin supply up to a given DAA score
    /// 
    /// This function calculates the total number of coins that have been issued up to
    /// a given DAA score by:
    /// - Summing complete halving periods (blocks_per_period * subsidy for that period)
    /// - Adding partial current period
    /// - Using saturating arithmetic to prevent overflow
    /// 
    /// Parameters:
    /// - daa_score: The DAA score to calculate total supply up to
    /// 
    /// Returns:
    /// - The total coin supply in sompi
    fn calculate_total_supply(&self, daa_score: u64) -> u64 {
        let blocks_per_period = self.deflationary_phase_daa_score;
        // Initial subsidy is already in sompi units (1 ENX = 100000000 sompi)
        let initial_subsidy = self.pre_deflationary_phase_base_subsidy;
        let mut total = 0u64;
        
        // Calculate supply from all complete halving periods
        // For each period, multiply blocks in period by that period's subsidy
        let complete_periods = daa_score / blocks_per_period;
        for period in 0..std::cmp::min(complete_periods, 64) {
            let period_subsidy = initial_subsidy >> period;
            total = total.saturating_add(period_subsidy * blocks_per_period);
        }
        
        // Add supply from partial current period
        let remaining_blocks = daa_score % blocks_per_period;
        let current_subsidy = if complete_periods >= 64 {
            0
        } else {
            initial_subsidy >> complete_periods
        };
        total = total.saturating_add(current_subsidy * remaining_blocks);
...
/*
    This table was pre-calculated by calling `calcDeflationaryPeriodBlockSubsidyFloatCalc` (in entropyx-go) for all months until reaching 0 subsidy.
...
mod tests {
    ...
    fn calc_high_bps_total_rewards_delta() {
        ...
        println!("Total rewards: {} sompi => {} ENX", total_rewards, total_rewards / SOMPI_PER_ENX);
        println!("Total high bps rewards: {} sompi => {} ENX", total_high_bps_rewards, total_high_bps_rewards / SOMPI_PER_ENX);
        println!("Delta: {} sompi => {} ENX", delta, delta / SOMPI_PER_ENX as i64);
    }
    ...
    #[test]
    /// ENX-CHANGE: Test how many halving periods are needed to reach the maximum supply
    fn test_halving_periods_to_max_supply() {
        let coinbase = create_manager(&MAINNET_PARAMS);

        let mut period = 0;
        loop {
            // Calculate DAA score at end of this period
            let daa_score = period * coinbase.deflationary_phase_daa_score;
            
            // Get total supply at this point
            let total_supply = coinbase.calculate_total_supply(daa_score);
            println!("The {}th mining period total supply is {}, and the block subsidy is {}.", period + 1, total_supply, coinbase.calc_block_subsidy(daa_score));
            
            if total_supply >= MAX_SOMPI {
                println!("The maximum supply {} is reached after {} halving periods.", MAX_SOMPI, period);
                break;
            }
    ...
    #[test]
    fn test_halving_by_daa_score() {
        let coinbase = create_manager(&MAINNET_PARAMS);
        let daa_score = 380384;
        let subsidy = coinbase.calc_block_subsidy(daa_score);
        let total_supply = coinbase.calculate_total_supply(daa_score);
        println!("The subsidy at DAA score {} is {}", daa_score, subsidy);
        println!("The total supply at DAA score {} is {}", daa_score, total_supply);
    }
