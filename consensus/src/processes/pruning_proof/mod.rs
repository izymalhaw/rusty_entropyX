...
use entropyx_math::int::SignedInteger;
...
use entropyx_core::{debug, info, trace};
use entropyx_database::prelude::{CachePolicy, ConnBuilder, StoreResultEmptyTuple, StoreResultExtensions};
use entropyx_hashes::Hash;
use entropyx_pow::calc_block_level;
use entropyx_utils::{binary_heap::BinaryHeapExtensions, vec::VecExtensions};
use thiserror::Error;
...
impl PruningProofManager {
    ...
    pub fn import_pruning_points(&self, pruning_points: &[Arc<Header>]) {
        for (i, header) in pruning_points.iter().enumerate() {
            ...
            let state = entropyx_pow::State::new(header);
    ...
    pub fn validate_pruning_point_proof(&self, proof: &PruningPointProof) -> PruningImportResult<()> {
        ...
        let (db_lifetime, db) = entropyx_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
        ...
        for anticone_block in anticone.iter().copied() {
            ...
            for hash in ghostdag_chain {
                if let Entry::Vacant(e) = ghostdag_blocks.entry(hash) {
                    ...
                    // We fill `ghostdag_blocks` only for entropyx-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which entropyx-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),
