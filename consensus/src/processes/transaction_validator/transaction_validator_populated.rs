use crate::constants::{MAX_SOMPI, SEQUENXE_LOCK_TIME_DISABLED, SEQUENXE_LOCK_TIME_MASK};
use entropyx_consensus_core::{
    hashing::sighash::SigHashReusedValues,
    mass::Kip9Version,
    tx::{TransactionInput, VerifiableTransaction},
};
use entropyx_core::warn;
use entropyx_txscript::{get_sig_op_count, TxScriptEngine};
use entropyx_txscript_errors::TxScriptError;
...
impl TransactionValidator {
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx.populated_inputs().filter(|(input, _)| input.sequence & SEQUENXE_LOCK_TIME_DISABLED != SEQUENXE_LOCK_TIME_DISABLED).any(
            |(input, entry)| {
                // Given a sequence number, we apply the relative time lock
                // mask in order to obtain the time lock delta required before
                // this input can be spent.
                let relative_lock = (input.sequence & SEQUENXE_LOCK_TIME_MASK) as i64;
                ...
                //
                // Note: in the entropyx codebase there's a use in i64 in order to use the -1 value
...
mod tests {
    ...
    use entropyx_consensus_core::sign::sign;
    use entropyx_consensus_core::subnets::SubnetworkId;
    use entropyx_consensus_core::tx::{MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry};
    use entropyx_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
    use entropyx_txscript_errors::TxScriptError;
