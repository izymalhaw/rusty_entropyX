...
use entropyx_consensus_core::{blockhash::ORIGIN, header::Header, BlockHashMap, BlockHasher, BlockLevel};
...
mod tests {
    ...
    use entropyx_database::prelude::{ReadLock, StoreError, StoreResult};
    use entropyx_hashes::Hash;
    ...
    impl HeaderStoreReader for HeaderStoreMock {
        fn get_daa_score(&self, hash: entropyx_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_timestamp(&self, hash: entropyx_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_bits(&self, hash: entropyx_hashes::Hash) -> Result<u32, StoreError> {
            unimplemented!()
        }

        fn get_header(&self, hash: entropyx_hashes::Hash) -> Result<Arc<Header>, StoreError> {
            Ok(self.map.read().get(&hash).unwrap().header.clone())
        }

        fn get_compact_header_data(
            &self,
            hash: entropyx_hashes::Hash,
        ) -> Result<crate::model::stores::headers::CompactHeaderData, StoreError> {
            unimplemented!()
        }

        fn get_blue_score(&self, hash: entropyx_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_header_with_block_level(&self, hash: entropyx_hashes::Hash) -> Result<HeaderWithBlockLevel, StoreError> {
    ...
    impl RelationsStoreReader for RelationsStoreMock {
        fn get_parents(&self, hash: Hash) -> Result<entropyx_consensus_core::blockhash::BlockHashes, StoreError> {
            unimplemented!()
        }
