...
use entropyx_consensus_core::{
    acceptance_data::AcceptanceData, 
    api::args::{TransactionValidationArgs, TransactionValidationBatchArgs}, 
    block::{BlockTemplate, MutableBlock, TemplateBuildMode, TemplateTransactionSelector}, 
    blockstatus::BlockStatus::{StatusDisqualifiedFromChain, StatusUTXOValid}, 
    burn::{BurnFeeCalculator, BurnRecord}, // ENX-CHANGE-BURN:
    coinbase::MinerData, 
...
use entropyx_consensusmanager::SessionLock;
use entropyx_core::{debug, info, time::unix_now, trace, warn};
use entropyx_database::prelude::{StoreError, StoreResultEmptyTuple, StoreResultExtensions};
use entropyx_hashes::Hash;
use entropyx_muhash::MuHash;
use entropyx_notify::{events::EventType, notifier::Notify};

use super::errors::{PruningImportError, PruningImportResult};
use crossbeam_channel::{Receiver as CrossbeamReceiver, Sender as CrossbeamSender};
use itertools::Itertools;
use entropyx_consensus_core::tx::ValidatedTransaction;
use entropyx_utils::binary_heap::BinaryHeapExtensions;
...
pub struct VirtualStateProcessor {
    ...
    pub burn_fee_calculator: BurnFeeCalculator, // ENX-CHANGE-BURN:
    ...
    pub(super) burned_fees_store: Arc<DbBurnedFeesStore>, // ENX-CHANGE-BURN:
...
impl VirtualStateProcessor {
    ...
    pub fn new(
        ...
        Self {
            ...
            burn_fee_calculator: params.burn_fee_calculator.clone(), // ENX-CHANGE-BURN:
            ...
            burned_fees_store: storage.burned_fees_store.clone(), // ENX-CHANGE-BURN:
        }
    }
    ...
    fn commit_utxo_state(&self, current: Hash, mergeset_diff: UtxoDiff, multiset: MuHash, acceptance_data: AcceptanceData) {
        ...
        // let acceptance_data_clone = acceptance_data.clone();
        self.acceptance_data_store.insert_batch(&mut batch, current, Arc::new(acceptance_data)).unwrap();
        
        // // ENX-CHANGE-BURN:
        // // Burn fees logic
        // let header = self.headers_store.get_header(current).unwrap();
        // let current_daa_score = header.daa_score;
        // let required_burn_fee = self.burn_fee_calculator.get_current_burn_fee(current_daa_score);
       
        // // Compute the total burned fees
        // let mut total_burned = 0u64;
        // for (block_idx, mergeset_block) in acceptance_data_clone.iter().enumerate() {
        //     if mergeset_block.accepted_transactions.len() > 1 {
        //         debug!("[BURN]Processing mergeset block {} with {} transactions", block_idx, mergeset_block.accepted_transactions.len());
        //     }
            
        //     for accepted_tx in mergeset_block.accepted_transactions.iter() {
        //         // Check if it's a coinbase transaction
        //         if accepted_tx.index_within_block == 0 {
        //             debug!("[BURN]Skipping coinbase transaction at index 0");
        //             continue;
        //         }
                
        //         total_burned += required_burn_fee;
        //         debug!("[BURN]Added burn fee for transaction at index {}. Transaction ID: {}. Current total: {}", 
        //             accepted_tx.index_within_block, accepted_tx.transaction_id, total_burned);
        //     }
        // }

        // // Store the total burned fees
        // if total_burned > 0 && self.reachability_service.is_chain_ancestor_of(virtual_selected_parent, current) {
        //     // Determine the parent hash
        //     // let parent_hash = self.headers_store.get_header(current).unwrap().parents_by_level[0][0];
        //     let burn_daa_score = current_daa_score.clone();
            
        //     if let Ok(Some(db_burn_record)) = self.burned_fees_store.get_burn_record(current) {
        //         info!("[BURN]Burn record already exists for block {} daa score {}, total burned: {}", 
        //             db_burn_record.block_hash, 
        //             db_burn_record.block_daa_score, 
        //             db_burn_record.total_burned
        //         );
        //     } else {
        //         match self.burned_fees_store.get_latest_burn_record() {
        //             Ok(Some(latest)) => {
        //                 let new_total = latest.total_burned.checked_add(total_burned)
        //                     .ok_or_else(|| warn!("[BURN]Burn fee total overflow"))
        //                     .unwrap();
                        
        //                 info!("[BURN]Processing burn fees for block {} with DAA score {}, unit burn fee: {}, utxo total burned: {}", current, burn_daa_score, required_burn_fee, total_burned);
        //                 info!("[BURN]Previous total burned: {}, New total burned: {}", latest.total_burned, new_total);

        //                 let burn_record = BurnRecord {
        //                     block_hash: current,
        //                     block_daa_score: burn_daa_score,
        //                     burned_value: total_burned,
        //                     total_burned: new_total,
        //                     timestamp: header.timestamp,
        //                 };
                        
        //                 if let Err(e) = self.burned_fees_store.insert_burn_record(burn_record) {
        //                     warn!("[BURN]Failed to store burn record: {}", e);
        //                 }
        //             }
        //             Ok(None) => {
        //                 // Handle the first record
        //                 info!("[BURN]Creating initial burn record for block {} with DAA score {}, unit burn fee: {}, utxo total burned: {}", current, burn_daa_score, required_burn_fee, total_burned);
        //                 let burn_record = BurnRecord {
        //                     block_hash: current,
        //                     block_daa_score: burn_daa_score,
        //                     burned_value: total_burned,
        ...
        //                 };
        //                 if let Err(e) = self.burned_fees_store.insert_burn_record(burn_record) {
        //                     warn!("[BURN]Failed to store first burn record: {}", e);
        //                 }
        //             }
        //             Err(e) => {
        //                 warn!("[BURN]Failed to get latest burn record: {}", e);
        //             }
        //         }
        //     }
        // }
    ...
    fn commit_virtual_state(
        ...
        // ENX-CHANGE-BURN: Begin 
        // let (_, selected_tip) = selected_chain_write.get_tip().unwrap();
        // Record burn fees for added blocks in the chain path
        for added_block in chain_path.added.iter() {
            // ENX-CHANGE-BURN: save the block burn records
            if let Some(burn_fee) = self.burned_fees_store.get_temp_burn_record(added_block) {
                self.record_burn_fees_with_total(added_block, burn_fee); // , &selected_tip
                // after processing, delete the temp record
                self.burned_fees_store.remove_temp_burn_record(added_block);
            }
            // ENX-CHANGE-BURN: End
        }
        // ENX-CHANGE-BURN: End
    ...
    /// ENX-CHANGE-BURN:
    /// record burn fees with total burned
    fn record_burn_fees_with_total(&self, block_hash: &Hash, total_burned: u64) { //, latest_hash: &Hash
        let header = self.headers_store.get_header(*block_hash).unwrap();
        
        // Get chain parent block hash of the latest block
        // let mut chain_parent = None;
        // if let Some(parents) = header.parents_by_level.get(0) {
        //     for parent in parents {
        //         // Use the reachability service to check if the parent is an ancestor of the latest block
        //         if self.reachability_service.is_chain_ancestor_of(*parent, *latest_hash) {
        //             chain_parent = Some(*parent);
        //             break;
        //         }
        //     }
        // }

        // if let Some(current) = chain_parent {
            // Get the parent block of the current block
            // let parent_header = self.headers_store.get_header(current).unwrap();
        let current = *block_hash;
        let burn_daa_score = header.daa_score.clone(); // parent_header.daa_score.clone();
        let required_burn_fee = self.burn_fee_calculator.get_current_burn_fee(burn_daa_score);
        ...
        if let Ok(Some(db_burn_record)) = self.burned_fees_store.get_burn_record(current) {
            ...
            match self.burned_fees_store.get_latest_burn_record() {
                ...
                Ok(None) => {
                    info!("[BURN]No previous burn record found, creating first record");
                    let burn_record = BurnRecord {
                        block_hash: current,
                        block_daa_score: burn_daa_score,
                        burned_value: total_burned,
    ...
    pub(crate) fn build_block_template_from_virtual_state(
        ...
        let accepted_id_merkle_root = entropyx_merkle::calc_merkle_root(virtual_state.accepted_tx_ids.iter().copied());
    ...
    // ENX-CHANGE-BURN: Record burn fees for added blocks
    fn record_burn_fees(&self, block_hash: &Hash, acceptance_data: &AcceptanceData, latest_hash: &Hash) {
        let header = self.headers_store.get_header(*block_hash).unwrap();
        ...
        let mut total_burned = 0u64;
        for (block_idx, mergeset_block) in acceptance_data.iter().enumerate() {
            if mergeset_block.accepted_transactions.len() > 1 {
                debug!("[BURN]Processing mergeset block {} with {} transactions", block_idx, mergeset_block.accepted_transactions.len());
            }
        ...
        if total_burned > 0 {
            ...
            if let Some(current) = chain_parent {
                // Get the parent block of the current block
                let parent_header = self.headers_store.get_header(current).unwrap();
                let burn_daa_score = parent_header.daa_score.clone();
                ...
                // 没有找到主链上的父区块，记录警告
                warn!("[BURN]No chain parent found for block {}", block_hash);
            }
        }
    }
}
