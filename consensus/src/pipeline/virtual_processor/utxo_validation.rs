...
use crate::{
    ...
    model::stores::{block_transactions::BlockTransactionsStoreReader, burned_fees::TempBurnRecordStore, daa::DaaStoreReader, ghostdag::GhostdagData},
...
use entropyx_core::{info, trace, warn};
use entropyx_hashes::Hash;
use entropyx_muhash::MuHash;
use entropyx_utils::refs::Refs;
...
impl VirtualStateProcessor {
    ...
    pub(super) fn calculate_utxo_state<V: UtxoView + Sync>(
            ...
            .enumerate()
        {
            ...
            for (validated_tx, _) in validated_transactions.iter() {
                ...
                // ENX-CHANGE-BURN: Only the actual fee is given to the miner
                let required_burn_fee = if self.transaction_validator.is_same_address_transaction(validated_tx) {
                    0
                } else {
                    self.burn_fee_calculator.get_current_burn_fee(pov_daa_score)
                };
                block_burn_fee += required_burn_fee;
                let actual_fee = validated_tx.calculated_fee - required_burn_fee;
                block_fee += actual_fee;
                // ENX-CHANGE-BURN: Only the actual fee is given to the miner
            }

            // ENX-CHANGE-BURN: Store the temp burn record for the current block
            if block_burn_fee > 0 {
                self.burned_fees_store.insert_temp_burn_record(merged_block, block_burn_fee);
            }
    ...
    pub(super) fn verify_expected_utxo_state<V: UtxoView + Sync>(
        ...
        let expected_accepted_id_merkle_root = entropyx_merkle::calc_merkle_root(ctx.accepted_tx_ids.iter().copied());
    ...
    pub(super) fn validate_mempool_transaction_in_utxo_context(
        ...
        // ENX-CHANGE-BURN: validate burn fee
        if !mutable_tx.tx.is_coinbase() {
            if !self.transaction_validator.is_same_address_transaction(&mutable_tx.as_verifiable()) {
                let required_burn_fee = self.burn_fee_calculator.get_current_burn_fee(pov_daa_score);
                let total_required_fee = required_burn_fee.saturating_add(contextual_mass);
            
                if calculated_fee < total_required_fee {
                    warn!(
                        "Consensus rejected mempool tx {} due to insufficient fee (provided: {}, required: {} = burn: {} + mass: {})",
                        mutable_tx.tx.id(),
                        calculated_fee,
                        total_required_fee,
                        required_burn_fee,
                    ...
                    return Err(TxRuleError::InsufficientFee {
                        tx_id: mutable_tx.tx.id(),
                        provided: calculated_fee,
                        burn_fee: required_burn_fee,
                        mass_based_fee: contextual_mass,
                    })
                }
            }
        }
