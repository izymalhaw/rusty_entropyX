...
use entropyx_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use entropyx_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use entropyx_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use entropyx_database::prelude::{DirectWriter, MemoryWriter};
use entropyx_database::prelude::{ReadLock, StoreError};
use entropyx_database::prelude::{StoreResult, DB};
use entropyx_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; entropyx_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use entropyx_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(entropyx_database::prelude::ConnBuilder::default().with_files_limit(10));
