...
use entropyx_database::prelude::{BatchDbWriter, CachedDbAccess, DirectDbWriter};
use entropyx_database::prelude::{CachePolicy, StoreError};
...
pub const UTXO_KEY_SIZE: usize = entropyx_hashes::HASH_SIZE + size_of::<TransactionIndexType>();
...
impl AsRef<[u8]> for UtxoKey {
    fn as_ref(&self) -> &[u8] {
        ...
        let rposition = self.0[entropyx_hashes::HASH_SIZE..].iter().rposition(|&v| v != 0).unwrap_or(0);
        &self.0[..=entropyx_hashes::HASH_SIZE + rposition]
    }
}
...
impl TryFrom<&[u8]> for UtxoKey {
    ...
    fn try_from(slice: &[u8]) -> Result<Self, Self::Error> {
        ...
        if slice.len() < entropyx_hashes::HASH_SIZE + 1 {
...
impl From<TransactionOutpoint> for UtxoKey {
    fn from(outpoint: TransactionOutpoint) -> Self {
        let mut bytes = [0; UTXO_KEY_SIZE];
        bytes[..entropyx_hashes::HASH_SIZE].copy_from_slice(&outpoint.transaction_id.as_bytes());
        bytes[entropyx_hashes::HASH_SIZE..].copy_from_slice(&outpoint.index.to_le_bytes());
        Self(bytes)
    }
}
...
impl From<UtxoKey> for TransactionOutpoint {
    fn from(k: UtxoKey) -> Self {
        let transaction_id = Hash::from_slice(&k.0[..entropyx_hashes::HASH_SIZE]);
        let index = TransactionIndexType::from_le_bytes(
            <[u8; size_of::<TransactionIndexType>()]>::try_from(&k.0[entropyx_hashes::HASH_SIZE..]).expect("expecting index size"),
        );
