...
use entropyx_hashes::{Hash, Hasher};

/// A bitmask defining which transaction fields we
/// want to encode and which to ignore.
type TxEncodingFlags = u8;

pub const TX_ENXODING_FULL: TxEncodingFlags = 0;
pub const TX_ENXODING_EXCLUDE_SIGNATURE_SCRIPT: TxEncodingFlags = 1;
...
pub fn hash(tx: &Transaction, include_mass_field: bool) -> Hash {
    let mut hasher = entropyx_hashes::TransactionHash::new();
    write_transaction(&mut hasher, tx, TX_ENXODING_FULL, include_mass_field);
    hasher.finalize()
}
...
pub(crate) fn id(tx: &Transaction) -> TransactionId {
    ...
    let encoding_flags = if tx.is_coinbase() { TX_ENXODING_FULL } else { TX_ENXODING_EXCLUDE_SIGNATURE_SCRIPT };
    let mut hasher = entropyx_hashes::TransactionID::new();
...
fn write_input<T: Hasher>(hasher: &mut T, input: &TransactionInput, encoding_flags: TxEncodingFlags) {
    ...
    if encoding_flags & TX_ENXODING_EXCLUDE_SIGNATURE_SCRIPT != TX_ENXODING_EXCLUDE_SIGNATURE_SCRIPT {
