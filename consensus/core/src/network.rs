...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'entropyx'.")]
...
///
/// NetworkId is a unique identifier for a entropyx network instance.
...
impl NetworkId {
    ...
    /// Returns a textual description of the network prefixed with `entropyx-`
    pub fn to_prefixed(&self) -> String {
        format!("entropyx-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("entropyx-") {
