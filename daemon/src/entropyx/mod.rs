...
impl Default for EntropyXConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<EntropyXConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: EntropyXConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no entropyx path is specified".to_string()));
        }
...
impl EntropyX {
    pub fn new(args: EntropyXConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: EntropyXConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("EntropyX node is already running.".to_string()));
            }
        }
        ...
        // let options = EntropyXOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("EntropyX binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait EntropyXCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: EntropyXConfig) -> Result<()>;
