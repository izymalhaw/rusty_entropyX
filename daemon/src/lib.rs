...
pub use entropyx::{EntropyX, EntropyXConfig, EntropyXCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../entropyx-cpu-miner/target/debug",
    "../../entropyx-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub entropyx: Option<Arc<dyn EntropyXCtl + Send + Sync + 'static>>,
    // pub entropyx_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { entropyx: None, cpu_miner: None }
    }

    pub fn with_entropyx(mut self, entropyx: Arc<dyn EntropyXCtl + Send + Sync + 'static>) -> Self {
        self.entropyx = Some(entropyx);
        self
    }
    ...
    pub fn entropyx(&self) -> Arc<dyn EntropyXCtl + Send + Sync + 'static> {
        self.entropyx.as_ref().expect("accessing Daemons::entropyx while entropyx option is None").clone()
    }

    pub fn try_entropyx(&self) -> Option<Arc<dyn EntropyXCtl + Send + Sync + 'static>> {
        self.entropyx.clone()
    }
