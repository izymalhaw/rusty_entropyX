    ...
    S: BuildHasher + Default,
{
    ...
    /// Reverse iteration and get the first record
    /// Since it is reverse iteration, the first record obtained is the last record
    pub fn seek_last(&self) -> Result<Option<(Box<[u8]>, TData)>, Box<dyn Error>>
    where
        TKey: Clone + AsRef<[u8]>,
        TData: DeserializeOwned,
    {
        ...
        let mut iterator = self.db.iterator_opt(
            IteratorMode::End,  // Start from the end
            read_opts
        );

        iterator.next().map(|result| {
            match result {
                Ok((key_bytes, value_bytes)) => {
                    match bincode::deserialize::<TData>(&value_bytes) {
                        Ok(value) => Ok((key_bytes[prefix_key.prefix_len()..].into(), value)),
                        Err(err) => Err(err.into()),
                    }
                }
                Err(err) => Err(err.into()),
