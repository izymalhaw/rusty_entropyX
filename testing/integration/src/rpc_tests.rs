...
use entropyx_addresses::{Address, Prefix, Version};
use entropyx_consensus::params::SIMNET_GENESIS;
use entropyx_consensus_core::{constants::MAX_SOMPI, header::Header, subnets::SubnetworkId, tx::Transaction};
...
use entropyx_rpc_core::{api::rpc::RpcApi, model::*, Notification};
use entropyx_utils::{fd_budget, networking::ContextualNetAddress};
use entropyx_lib::args::Args;
...
/// `cargo test --release --package entropyx-testing-integration --lib -- rpc_tests::sanity_test`
#[tokio::test]
async fn sanity_test() {
    ...
    for op in EntropyXPayloadOps::iter() {
        let network_id = daemon.network;
        let task: JoinHandle<()> = match op {
            EntropyXPayloadOps::SubmitBlock => {
            ...
            EntropyXPayloadOps::GetBlockTemplate => {
                tst!(op, "see SubmitBlock")
            }

            EntropyXPayloadOps::GetCurrentBlockColor => {
                tst!(op, "see SubmitBlock")
            }

            EntropyXPayloadOps::GetCurrentNetwork => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetInfo => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_info_call(None, GetInfoRequest {}).await.unwrap();
                    assert_eq!(response.server_version, entropyx_core::entropyx_env::version().to_string());
            ...
            EntropyXPayloadOps::GetMempoolEntries => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            EntropyXPayloadOps::GetConnectedPeerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::Unban => {
                tst!(op, "see Ban")
            }

            EntropyXPayloadOps::SubmitTransaction => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::SubmitTransactionReplacement => {
                let rpc_client = client.clone();
                tst!(op, {
                    // Build an erroneous transaction...
                    let transaction = Transaction::new(0, vec![], vec![], 0, SubnetworkId::default(), 0, vec![]);
            ...
            EntropyXPayloadOps::GetSubnetwork => {
                let rpc_client = client.clone();
                tst!(op, {
                    let result =
            ...
            EntropyXPayloadOps::GetVirtualChainFromBlock => {
                tst!(op, "see SubmitBlock")
            }

            EntropyXPayloadOps::GetBlockCount => {
                tst!(op, "see SubmitBlock")
            }

            EntropyXPayloadOps::GetBlockDagInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::ResolveFinalityConflict => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            EntropyXPayloadOps::GetUtxosByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetBalanceByAddress => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client
            ...
            EntropyXPayloadOps::GetBalancesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetSinkBlueScore => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response = rpc_client.get_sink_blue_score_call(None, GetSinkBlueScoreRequest {}).await.unwrap();
            ...
            EntropyXPayloadOps::EstimateNetworkHashesPerSecond => {
                let rpc_client = client.clone();
                tst!(op, {
                    let response_result = rpc_client
            ...
            EntropyXPayloadOps::GetMempoolEntriesByAddresses => {
                let rpc_client = client.clone();
                tst!(op, {
                    let addresses = vec![Address::new(Prefix::Simnet, Version::PubKey, &[0u8; 32])];
                    let response = rpc_client
            ...
            EntropyXPayloadOps::GetCoinSupply => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetConnections => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetSystemInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetServerInfo => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetSyncStatus => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetDaaScoreTimestampEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetFeeEstimate => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::GetFeeEstimateExperimental => {
                let rpc_client = client.clone();
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyBlockAdded => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyNewBlockTemplate => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyFinalityConflict => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifySinkBlueScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyVirtualDaaScoreChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::NotifyVirtualChainChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
                    rpc_client
            ...
            EntropyXPayloadOps::StopNotifyingUtxosChanged => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
            ...
            EntropyXPayloadOps::StopNotifyingPruningPointUtxoSetOverride => {
                let rpc_client = client.clone();
                let id = listener_id;
                tst!(op, {
