...
use entropyx_consensus::params::Params;
use entropyx_consensus_core::network::{NetworkId, NetworkType};
use entropyx_core::{info, task::tick::TickService, trace};
use entropyx_math::Uint256;
use entropyx_notify::scope::VirtualDaaScoreChangedScope;
use entropyx_rpc_core::api::rpc::RpcApi;
use entropyx_txscript::pay_to_address_script;
use rand::thread_rng;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), entropyx_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    entropyx_core::log::try_init_logger(
        "INFO, entropyx_core::time=debug, entropyx_rpc_core=debug, entropyx_grpc_client=debug, entropyx_notify=info, entropyx_notify::address::tracker=debug, entropyx_notify::listener=debug, entropyx_notify::subscription::single=debug, entropyx_mining::monitor=debug, entropyx_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    entropyx_core::panic::configure_panic();
    ...
    // Fold-up
    //
    entropyx_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    entropyx_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    entropyx_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=16610 --p2p=16611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    entropyx_core::log::try_init_logger(
        "INFO, entropyx_core::core=trace, entropyx_core::time=debug, entropyx_rpc_core=debug, entropyx_grpc_client=debug, entropyx_notify=info, entropyx_notify::address::tracker=debug, entropyx_notify::listener=debug, entropyx_notify::subscription::single=debug, entropyx_mining::monitor=debug, entropyx_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    entropyx_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), entropyx_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
...
/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package entropyx-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
