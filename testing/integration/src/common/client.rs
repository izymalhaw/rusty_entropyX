...
use entropyx_notify::{events::EventType, scope::Scope, subscription::Command};
use entropyx_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(entropyx: &Daemon) -> Self {
        let client = entropyx.new_multi_listener_client().await;
