...
use entropyx_consensus::config::genesis::GENESIS;
use entropyx_consensus::config::{Config, ConfigBuilder};
use entropyx_consensus::consensus::factory::Factory as ConsensusFactory;
use entropyx_consensus::consensus::test_consensus::{TestConsensus, TestConsensusFactory};
use entropyx_consensus::model::stores::block_transactions::{
    BlockTransactionsStore, BlockTransactionsStoreReader, DbBlockTransactionsStore,
};
use entropyx_consensus::model::stores::ghostdag::{GhostdagStoreReader, KType as GhostdagKType};
use entropyx_consensus::model::stores::headers::HeaderStoreReader;
use entropyx_consensus::model::stores::reachability::DbReachabilityStore;
use entropyx_consensus::model::stores::relations::DbRelationsStore;
use entropyx_consensus::model::stores::selected_chain::SelectedChainStoreReader;
use entropyx_consensus::params::{Params, DEVNET_PARAMS, MAINNET_PARAMS, MAX_DIFFICULTY_TARGET, MAX_DIFFICULTY_TARGET_AS_F64};
use entropyx_consensus::pipeline::monitor::ConsensusMonitor;
use entropyx_consensus::pipeline::ProcessingCounters;
use entropyx_consensus::processes::reachability::tests::{DagBlock, DagBuilder, StoreValidationExtensions};
use entropyx_consensus::processes::window::{WindowManager, WindowType};
use entropyx_consensus_core::api::{BlockValidationFutures, ConsensusApi};
use entropyx_consensus_core::block::Block;
use entropyx_consensus_core::blockhash::new_unique;
use entropyx_consensus_core::blockstatus::BlockStatus;
use entropyx_consensus_core::constants::{BLOCK_VERSION, STORAGE_MASS_PARAMETER};
use entropyx_consensus_core::errors::block::{BlockProcessResult, RuleError};
use entropyx_consensus_core::header::Header;
use entropyx_consensus_core::network::{NetworkId, NetworkType::Mainnet};
use entropyx_consensus_core::subnets::SubnetworkId;
use entropyx_consensus_core::trusted::{ExternalGhostdagData, TrustedBlock};
use entropyx_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry};
use entropyx_consensus_core::{blockhash, hashing, BlockHashMap, BlueWorkType};
use entropyx_consensus_notify::root::ConsensusNotificationRoot;
use entropyx_consensus_notify::service::NotifyService;
use entropyx_consensusmanager::ConsensusManager;
use entropyx_core::task::tick::TickService;
use entropyx_core::time::unix_now;
use entropyx_database::utils::get_entropyx_tempdir;
use entropyx_hashes::Hash;

use flate2::read::GzDecoder;
use futures_util::future::try_join_all;
use itertools::Itertools;
use entropyx_core::core::Core;
use entropyx_core::signals::Shutdown;
use entropyx_core::task::runtime::AsyncRuntime;
use entropyx_core::{assert_match, info};
use entropyx_database::create_temp_db;
use entropyx_database::prelude::{CachePolicy, ConnBuilder};
use entropyx_index_processor::service::IndexService;
use entropyx_math::Uint256;
use entropyx_muhash::MuHash;
use entropyx_notify::subscription::context::SubscriptionContext;
use entropyx_txscript::caches::TxScriptCacheCounters;
use entropyx_utxoindex::api::{UtxoIndexApi, UtxoIndexProxy};
use entropyx_utxoindex::UtxoIndex;
...
async fn json_test(file_path: &str, concurrency: bool) {
    entropyx_core::log::try_init_logger("info");
    ...
    let go_params_res: Result<EntropyXGoParams, _> = serde_json::from_str(&first_line);
...
async fn staging_consensus_test() {
    ...
    let db_tempdir = get_entropyx_tempdir();
    let db_path = db_tempdir.path().to_owned();
    let consensus_db_dir = db_path.join("consensus");
    let meta_db_dir = db_path.join("meta");

    let meta_db = entropyx_database::prelude::ConnBuilder::default().with_db_path(meta_db_dir).with_files_limit(5).build().unwrap();
