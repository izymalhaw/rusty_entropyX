...
use entropyx_consensus::params::SIMNET_PARAMS;
use entropyx_consensus_core::header::Header;
use entropyx_consensusmanager::ConsensusManager;
use entropyx_core::{task::runtime::AsyncRuntime, trace};
use entropyx_grpc_client::GrpcClient;
use entropyx_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use entropyx_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  entropyx_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut entropyx1 = Daemon::new_random(total_fd_limit);
    ...
    let mut entropyx2 = Daemon::new_random(total_fd_limit);
...
async fn daemon_mining_test() {
    ...
    let mut entropyx1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut entropyx2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = entropyx1.start().await;
    let rpc_client2 = entropyx2.start().await;
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(entropyx1.network.into(), entropyx_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            entropyx_consensus::params::SIMNET_GENESIS.hash, //
            true,
        )
        .await
        .unwrap();
...
/// `cargo test --release --package entropyx-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("entropyx-testing-integration-heap.json").build();

    entropyx_core::log::try_init_logger(
        "INFO,entropyx_testing_integration=trace,entropyx_notify=debug,entropyx_rpc_core=debug,entropyx_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(entropyx1.network.into(), entropyx_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(entropyx1.network.into(), entropyx_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(entropyx1.network.into(), entropyx_addresses::Version::PubKey, &[0; 32]);
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(entropyx_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&entropyx2).await, ListeningClient::connect(&entropyx1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    entropyx_core::log::try_init_logger("info,entropyx_grpc_core=trace,entropyx_grpc_server=trace,entropyx_grpc_client=trace,entropyx_core=trace");
    ...
    {
        let total_fd_limit = 10;
        let mut entropyx1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = entropyx1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = entropyx1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&entropyx1.core);
