use crate::cli::EntropyXCli;
use crate::imports::*;
use crate::result::Result;
use entropyx_bip32::{Language, Mnemonic, WordCount};
use entropyx_wallet_core::account::MULTISIG_ACCOUNT_KIND;
// use entropyx_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use entropyx_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use entropyx_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<EntropyXCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<EntropyXCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<EntropyXCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
