use crate::imports::*;
use entropyx_daemon::{locate_binaries, CpuMinerConfig};
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<EntropyXCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<EntropyXCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<EntropyXCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = entropyx_daemon::locate_binaries(root.as_str(), "entropyx-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No entropyx-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select entropyx-cpu-miner binary", &binaries).await? {
