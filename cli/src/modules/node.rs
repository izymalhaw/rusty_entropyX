use crate::imports::*;
use entropyx_daemon::EntropyXConfig;
...
#[async_trait]
impl DefaultSettings for EntropyXSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        let mut settings = vec![(Self::Mute, to_value(true).unwrap())];

        let root = nw_sys::app::folder();
        if let Ok(binaries) = entropyx_daemon::locate_binaries(&root, "entropyx").await {
...
pub struct Node {
    settings: SettingsStore<EntropyXSettings>,
    mute: Arc<AtomicBool>,
...
impl Default for Node {
    fn default() -> Self {
        Node {
            settings: SettingsStore::try_new("entropyx").expect("Failed to create node settings store"),
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<EntropyXCli>() {
            ctx.daemons().clone().entropyx.as_ref().map(|_| "node")
        } else {
            None
        }
    }
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Manage the local EntropyX node instance"
    }

    async fn start(self: Arc<Self>, _ctx: &Arc<dyn Context>) -> cli::Result<()> {
        self.settings.try_load().await.ok();
        if let Some(mute) = self.settings.get(EntropyXSettings::Mute) {
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<EntropyXCli>) -> Result<EntropyXConfig> {
        let location: String = self
            .settings
            .get(EntropyXSettings::Location)
        ...
        let mute = false;
        let config = EntropyXConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<EntropyXCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let entropyx = ctx.daemons().entropyx();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting entropyx node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting entropyx node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                entropyx.configure(self.create_config(&ctx).await?).await?;
                entropyx.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // entropyx.mute(mute).await?;
                self.settings.set(EntropyXSettings::Mute, mute).await?;
            }
            "status" => {
                let status = entropyx.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                entropyx.configure(self.create_config(&ctx).await?).await?;
                let version = entropyx.version().await?;
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<EntropyXCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("select", "Select EntropyX executable (binary) location"),
                ("version", "Display EntropyX executable version"),
                ("start", "Start the local EntropyX node instance"),
                ("stop", "Stop the local EntropyX node instance"),
                ("restart", "Restart the local EntropyX node instance"),
                ("kill", "Kill the local EntropyX node instance"),
                ("status", "Get the status of the local EntropyX node instance"),
    ...
    async fn select(self: Arc<Self>, ctx: Arc<EntropyXCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = entropyx_daemon::locate_binaries(root.as_str(), "entropyx").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No entropyx binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a entropyx binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(EntropyXSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(EntropyXSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/entropyx'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<EntropyXCli>, event: Event) -> Result<()> {
        let term = ctx.term();
        ...
        match event {
            ...
            Event::Exit(_code) => {
                tprintln!(ctx, "EntropyX has exited");
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
            Event::Error(error) => {
                tprintln!(ctx, "{}", style(format!("EntropyX error: {error}")).red());
                self.is_running.store(false, Ordering::SeqCst);
                term.refresh_prompt();
            }
