...
use entropyx_consensus_core::tx::{TransactionOutpoint, UtxoEntry};
use entropyx_wallet_core::account::pskb::finalize_pskt_one_or_more_sig_and_redeem_script;
use entropyx_wallet_pskt::{
    prelude::{lock_script_sig_templating, script_sig_to_address, unlock_utxos_as_pskb, Bundle, Signer, PSKT},
    pskt::Inner,
};

#[derive(Default, Handler)]
#[help("Send a EntropyX transaction to a public address")]
pub struct Pskb;
    ...
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        match action.as_str() {
            "create" => {
                ...
                let amount_sompi = try_parse_required_nonzero_entropyx_as_sompi_u64(argv.get(1))?;
                let outputs = PaymentOutputs::from((address, amount_sompi));
                let priority_fee_sompi = try_parse_optional_entropyx_as_sompi_i64(argv.get(2))?.unwrap_or(0);
            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    "lock" => {
                        let amount_sompi = try_parse_required_nonzero_entropyx_as_sompi_u64(argv.first())?;
                        let outputs = PaymentOutputs::from((script_p2sh, amount_sompi));
                        let priority_fee_sompi = try_parse_optional_entropyx_as_sompi_i64(argv.get(1))?.unwrap_or(0);
                        let abortable = Abortable::default();
                    ...
                    "unlock" => {
                        ...
                        // Get locked UTXO set.
                        let spend_utxos: Vec<entropyx_rpc_core::RpcUtxosByAddressesEntry> =
                            ctx.wallet().rpc_api().get_utxos_by_addresses(vec![script_p2sh.clone()]).await?;
                        let priority_fee_sompi = try_parse_optional_entropyx_as_sompi_i64(argv.first())?.unwrap_or(0) as u64;
                        ...
                        tprintln!(
                            ctx,
                            "{} locked UTXO{} found with total amount of {} ENX",
                            spend_utxos.len(),
                            if spend_utxos.len() == 1 { "" } else { "s" },
                            sompi_to_entropyx(total_locked_sompi)
                        );
            ...
            "parse" => {
                ...
                let pskb = Self::parse_input_pskb(argv.first().unwrap().as_str())?;
                tprintln!(ctx, "{}", pskb.display_format(ctx.wallet().network_id()?, sompi_to_entropyx_string_with_suffix));
