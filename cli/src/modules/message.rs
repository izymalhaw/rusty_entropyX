use entropyx_addresses::Version;
use entropyx_bip32::secp256k1::XOnlyPublicKey;
use entropyx_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<EntropyXCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, entropyx_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<EntropyXCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <entropyx_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <entropyx_address> <signature>",
                    "Verify the signature against the message and entropyx_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<EntropyXCli>, entropyx_address: &str, message: &str) -> Result<()> {
        let entropyx_address = Address::try_from(entropyx_address)?;
        if entropyx_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, entropyx_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<EntropyXCli>, entropyx_address: &str, signature: &str, message: &str) -> Result<()> {
        let entropyx_address = Address::try_from(entropyx_address)?;
        if entropyx_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&entropyx_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<EntropyXCli>, entropyx_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&entropyx_address])?;
