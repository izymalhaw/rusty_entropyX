use crate::imports::*;
use entropyx_wallet_core::account::{multisig::MultiSig, Account, BIP32_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND};
...
async fn export_multisig_account(ctx: Arc<EntropyXCli>, account: Arc<MultiSig>) -> Result<()> {
    match &account.prv_key_data_ids() {
        ...
        Some(prv_key_data_ids) => {
            ...
            for (id, prv_key_data_id) in prv_key_data_ids.iter().enumerate() {
                ...
                let xpub_key: entropyx_bip32::ExtendedPublicKey<entropyx_bip32::secp256k1::PublicKey> =
...
async fn export_single_key_account(ctx: Arc<EntropyXCli>, account: Arc<dyn Account>) -> Result<()> {
