...
use entropyx_daemon::{DaemonEvent, DaemonKind, Daemons};
use entropyx_wallet_core::account::Account;
use entropyx_wallet_core::rpc::DynRpcApi;
use entropyx_wallet_core::storage::{IdT, PrvKeyDataInfo};
use entropyx_wrpc_client::{EntropyXRpcClient, Resolver};
...
impl From<&EntropyXCli> for Arc<Terminal> {
    fn from(ctx: &EntropyXCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<EntropyXCli> for EntropyXCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for EntropyXCli {
...
impl EntropyXCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                entropyx_core::log::init_logger(None, "info");
            } else {
                entropyx_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let entropyx_cli = Arc::new(EntropyXCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(entropyx_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                entropyx_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<EntropyXRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("EntropyX NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("EntropyX NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: EntropyX node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to EntropyX node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - EntropyX node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "EntropyX node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let entropyx = self.daemons().try_entropyx();
            ...
            if let Some(entropyx) = entropyx.as_ref() {
                entropyx.mute(false).await?;
                entropyx.stop().await?;
            }
...
impl Cli for EntropyXCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "entropyx" {
                prompt.push(title);
            }
...
impl cli::Context for EntropyXCli {
...
pub async fn entropyx_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    EntropyXCli::init();

    let options = Options::new(terminal_options, None);
    let cli = EntropyXCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("EntropyX Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));
