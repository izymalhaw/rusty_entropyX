...
use entropyx_consensus_core::hashing::sighash::{calc_ecdsa_signature_hash, calc_schnorr_signature_hash, SigHashReusedValues};
use entropyx_consensus_core::hashing::sighash_type::SigHashType;
use entropyx_consensus_core::tx::{ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
...
pub const MAX_TX_IN_SEQUENXE_NUM: u64 = u64::MAX;
pub const SEQUENXE_LOCK_TIME_DISABLED: u64 = 1 << 63;
pub const SEQUENXE_LOCK_TIME_MASK: u64 = 0x00000000ffffffff;
...
impl<'a, T: VerifiableTransaction> TxScriptEngine<'a, T> {
    ...
    fn execute_opcode(&mut self, opcode: Box<dyn OpCodeImplementation<T>>) -> Result<(), TxScriptError> {
        // Different from entropyx: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
...
mod bitcoind_tests {
    ...
    use entropyx_consensus_core::constants::MAX_TX_IN_SEQUENXE_NUM;
    use entropyx_consensus_core::tx::{
        PopulatedTransaction, ScriptPublicKey, Transaction, TransactionId, TransactionOutpoint, TransactionOutput,
    };
