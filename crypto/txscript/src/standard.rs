...
use entropyx_consensus_core::tx::{ScriptPublicKey, ScriptVec};
...
pub mod test_helpers {
    use super::*;
    use crate::{opcodes::codes::OpTrue, MAX_TX_IN_SEQUENXE_NUM};
        ...
        for tx_to_spend in txs_to_spend {
            for i in output_indexes.iter().copied() {
                if i < tx_to_spend.outputs.len() {
                    ...
                    inputs.push(TransactionInput::new(previous_outpoint, signature_script.clone(), MAX_TX_IN_SEQUENXE_NUM, 1));
    ...
    fn test_extract_address_and_encode_script() {
        ...
        let tests = vec![
            Test {
                ...
                expected_address: Ok("entropyx:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j".try_into().unwrap()),
            },
            Test {
                ...
                expected_address: Ok("entropyxtest:qxaqrlzlf6wes72en3568khahq66wf27tuhfxn5nytkd8tcep2c0vrse6gdmpks".try_into().unwrap()),
            },
            Test {
