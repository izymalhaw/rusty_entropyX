...
use crate::{
    ScriptSource, TxScriptEngine, TxScriptError, LOCK_TIME_THRESHOLD, MAX_TX_IN_SEQUENXE_NUM, NO_COST_OPCODE,
    SEQUENXE_LOCK_TIME_DISABLED, SEQUENXE_LOCK_TIME_MASK,
};
use blake2b_simd::Params;
use core::cmp::{max, min};
use entropyx_consensus_core::hashing::sighash_type::SigHashType;
use entropyx_consensus_core::tx::VerifiableTransaction;
...
opcode_list! {
    ...
    opcode OpIf<0x63, 1>(self, vm) {
        ...
        if vm.is_executing() {
            ...
            // the similar flow of go-entropyx
    ...
    opcode OpCheckLockTimeVerify<0xb0, 1>(self, vm) {
        match vm.script_source {
            ScriptSource::TxInput {input, tx, ..} => {
                ...
                if input.sequence == MAX_TX_IN_SEQUENXE_NUM {
    ...
    opcode OpCheckSequenceVerify<0xb1, 1>(self, vm) {
        match vm.script_source {
            ScriptSource::TxInput {input, tx, ..} => {
                ...
                // CHECKSEQUENXEVERIFY behaves as a NOP.
                if stack_sequence & SEQUENXE_LOCK_TIME_DISABLED != 0 {
                    return Ok(());
                }
                ...
                // to get around a CHECKSEQUENXEVERIFY check.
                if input.sequence & SEQUENXE_LOCK_TIME_DISABLED != 0 {
                ...
                if (stack_sequence & SEQUENXE_LOCK_TIME_MASK) > (input.sequence & SEQUENXE_LOCK_TIME_MASK) {
                    return Err(TxScriptError::UnsatisfiedLockTime(format!("locktime requirement not satisfied -- locktime is greater than the transaction locktime: {} > {}", stack_sequence & SEQUENXE_LOCK_TIME_MASK, input.sequence & SEQUENXE_LOCK_TIME_MASK)))
                }
                Ok(())
            }
            _ => Err(TxScriptError::InvalidSource("LockTimeVerify only applies to transaction inputs".to_string()))
...
mod test {
    ...
    use entropyx_consensus_core::constants::{SOMPI_PER_ENX, TX_VERSION};
    use entropyx_consensus_core::hashing::sighash::SigHashReusedValues;
    use entropyx_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use entropyx_consensus_core::tx::{
    ...
    fn make_mock_transaction(lock_time: u64) -> (VerifiableTransactionMock, TransactionInput, UtxoEntry) {
        let dummy_prev_out = TransactionOutpoint::new(entropyx_hashes::Hash::from_u64_word(1), 1);
        ...
        let dummy_tx_out = TransactionOutput::new(SOMPI_PER_ENX, dummy_script_public_key);
