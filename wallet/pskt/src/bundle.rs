...
// use entropyx_bip32::Prefix;
use entropyx_consensus_core::network::{NetworkId, NetworkType};
use entropyx_consensus_core::tx::{ScriptPublicKey, TransactionOutpoint, UtxoEntry};

use hex;
use entropyx_txscript::{extract_script_pub_key_address, pay_to_address_script, pay_to_script_hash_script};
...
pub fn script_sig_to_address(script_sig: &[u8], prefix: entropyx_addresses::Prefix) -> Result<Address, Error> {
...
mod tests {
    ...
    use crate::role::*;
    use entropyx_consensus_core::tx::{TransactionId, TransactionOutpoint, UtxoEntry};
    use entropyx_txscript::{multisig_redeem_script, pay_to_script_hash_script};
