...
use entropyx_wallet_pskt::bundle::Bundle;
pub use kind::*;
use pskb::{
...
use entropyx_bip32::{ChildNumber, ExtendedPrivateKey, PrivateKey};
use entropyx_consensus_client::UtxoEntryReference;
use entropyx_wallet_keys::derivation::gen0::WalletDerivationManagerV0;
...
pub trait Account: AnySync + Send + Sync + 'static {
    ...
    async fn sweep(
    ...
    ) -> Result<(GeneratorSummary, Vec<entropyx_hashes::Hash>)> {
        let keydata = self.prv_key_data(wallet_secret).await?;
        let signer = Arc::new(Signer::new(self.clone().as_dyn_arc(), keydata.clone(), payment_secret.clone()));
        
        let mut all_ids = vec![];
        let mut summaries = vec![];
    
        // ENX-CHANGE-BURN:
        // 1. first sweep change_address utxos to change_address
        let change_settings = GeneratorSettings::try_new_for_sweep(
        ...
        let change_generator = Generator::try_new(change_settings, Some(signer.clone()), Some(abortable))?;
        let mut change_stream = change_generator.stream();
        while let Some(transaction) = change_stream.try_next().await? {
            transaction.try_sign()?;
            all_ids.push(transaction.try_submit(&self.wallet().rpc_api()).await?);
        ...
        summaries.push(change_generator.summary());
    
        // 2. second sweep receive_address utxos to receive_address
        let receive_settings = GeneratorSettings::try_new_for_sweep(
            self.clone().as_dyn_arc(),
            self.receive_address()?,
            PaymentDestination::Receive,
            Fees::None,
        )?;
        
        let receive_generator = Generator::try_new(receive_settings, Some(signer), Some(abortable))?;
        let mut receive_stream = receive_generator.stream();
        while let Some(transaction) = receive_stream.try_next().await? {
            transaction.try_sign()?;
            all_ids.push(transaction.try_submit(&self.wallet().rpc_api()).await?);
        ...
        summaries.push(receive_generator.summary());
    
        // combine summaries
        let final_summary = GeneratorSummary::combine(summaries);
        Ok((final_summary, all_ids))
    }

    /// ENX-CHANGE-BURN: Send funds with burn fee optimization by selectively sweeping only the necessary UTXOs
    async fn send(
        self: Arc<Self>,
        ...
        // 1. First, estimate if this transaction would generate multiple transactions
        let estimate_summary = self.clone().estimate(
            destination.clone(), 
        ...
        // If only one transaction would be generated, proceed with normal send
        if estimate_summary.number_of_generated_transactions <= 1 {
            return self.clone().send_original(
                destination,
                priority_fee_sompi,
                payload,
                wallet_secret,
        ...
        // 2. Get transaction amount needed
        let required_amount = match &destination {
            PaymentDestination::PaymentOutputs(outputs) => {
                outputs.iter().map(|output| output.amount).sum::<u64>()
            },
            PaymentDestination::Change => {
                // For change sweeps, we don't need to calculate
                return self.clone().send_original(
                    destination,
                    priority_fee_sompi,
                    payload,
            ...
            PaymentDestination::Receive => {
                // For receive sweeps, we don't need to calculate
                return self.clone().send_original(
                    destination,
                    priority_fee_sompi,
                    payload,
        ...
        let fee_estimate = match &priority_fee_sompi {
            Fees::SenderPays(fee) => *fee,
            Fees::ReceiverPays(_) => 0, // Receiver pays fees
            Fees::None => 0,
        };
        
        // Get the burn fee for the transaction
        let burn_fee = self.wallet().utxo_processor().get_current_burn_fee();
        
        // Calculate total amount needed including fees and some buffer for transaction fees
        let total_required = required_amount + fee_estimate + burn_fee + 1000; // Add a small buffer for tx fees
        
        // 3. Analyze UTXOs to find the most efficient set to sweep
        let mut all_ids = vec![];
        let mut summaries = vec![];
        
        // Get the keydata and signer ready for all operations
        let keydata = self.prv_key_data(wallet_secret.clone()).await?;
        let signer = Arc::new(Signer::new(self.clone().as_dyn_arc(), keydata.clone(), payment_secret.clone()));
        
        // 4. Perform targeted sweeps
        let mut already_swept_amount = 0;
        let mut sweep_performed = false;
        
        // First check receive address UTXOs
        let receive_utxos = self.utxo_context().context().mature.iter()
            .filter(|utxo| utxo.address().as_ref() == Some(&self.receive_address().unwrap()))
            .cloned()
            .collect::<Vec<_>>();
        
        if !receive_utxos.is_empty() {
            let receive_total = receive_utxos.iter().map(|utxo| utxo.amount()).sum::<u64>();
            already_swept_amount += receive_total;
            
            // If there are multiple receive UTXOs, sweep them
            if receive_utxos.len() > 1 {
                ...
                let receive_generator = Generator::try_new(receive_settings, Some(signer.clone()), Some(abortable))?;
                let mut receive_stream = receive_generator.stream();
                while let Some(transaction) = receive_stream.try_next().await? {
                    transaction.try_sign()?;
                    all_ids.push(transaction.try_submit(&self.wallet().rpc_api()).await?);
        ...
        // If we still need more funds, sweep change address UTXOs
        if already_swept_amount < total_required {
            let change_utxos = self.utxo_context().context().mature.iter()
                .filter(|utxo| utxo.address().as_ref() == Some(&self.change_address().unwrap()))
                .cloned()
                .collect::<Vec<_>>();
            
            if !change_utxos.is_empty() {
                let change_total = change_utxos.iter().map(|utxo| utxo.amount()).sum::<u64>();
                already_swept_amount += change_total;
                
                // If there are multiple change UTXOs, sweep them
                if change_utxos.len() > 1 {
                    sweep_performed = true;
                    let change_settings = GeneratorSettings::try_new_for_sweep(
                        self.clone().as_dyn_arc(),
                        self.change_address()?,
        ...
        if already_swept_amount < total_required {
            return Err(Error::InsufficientFunds { additional_needed: total_required - already_swept_amount, origin: "send_with_burn_fee_optimization" });
        }
        
        // If we swept UTXOs, wait for balance to be sufficient for the transfer
        if sweep_performed {          
            ...
            loop {
                // Create a fresh clone for scan
                {
                    let scan_clone = self.clone();
                    scan_clone.scan(None, None).await?;
                }
                
                // Create a separate clone for balance check
                let balance_clone = self.clone();
                let current_balance = balance_clone.utxo_context().context().mature.iter()
                    .map(|utxo| utxo.amount())
                    .sum::<u64>();
                
                if current_balance >= total_required {
                    // Balance is sufficient, proceed with the send operation
                    break;
                }
                
                attempts += 1;
                if attempts >= MAX_ATTEMPTS {
                    // Too many attempts, return error
                    return Err(Error::custom(
                        format!("Balance not sufficient after waiting for sweep operations to complete. Current balance: {}, Required: {}. Please try again in a few moments.", current_balance, total_required)
                    ));
                }
                
                // Wait for 3 second using our new delay function
                delay_seconds(3).await?;
            }
        }
        
        // 5. Now send the actual transaction
        let send_settings = GeneratorSettings::try_new_with_account(
            self.clone().as_dyn_arc(), 
            destination, 
            priority_fee_sompi, 
            payload
        ...
        let send_generator = Generator::try_new(send_settings, Some(signer), Some(abortable))?;
        
        let mut send_stream = send_generator.stream();
        let mut send_ids = vec![];
        while let Some(transaction) = send_stream.try_next().await? {
            transaction.try_sign()?;
            send_ids.push(transaction.try_submit(&self.wallet().rpc_api()).await?);
        ...
        // Add send results to our tracking
        all_ids.extend(send_ids);
        summaries.push(send_generator.summary());
        
        // 6. Combine the summaries
        let final_summary = if summaries.len() > 1 {
            GeneratorSummary::combine(summaries)
        } else {
            summaries[0].clone()
        };
    ...
    // ENX-CHANGE-BURN: Send funds to a [`PaymentDestination`] comprised of one or multiple [`PaymentOutputs`](crate::tx::PaymentOutputs)
...
// ENX-CHANGE-BURN: delay_seconds
async fn delay_seconds(seconds: u64) -> Result<()> {
...
mod tests {
    ...
    use entropyx_bip32::secp256k1::SecretKey;
    use entropyx_bip32::PrivateKey;
    use entropyx_bip32::SecretKeyExt;
    use entropyx_wallet_keys::derivation::gen0::PubkeyDerivationManagerV0;
    use std::str::FromStr;

    fn gen0_receive_addresses() -> Vec<&'static str> {
        vec![
            "entropyxtest:qqnapngv3zxp305qf06w6hpzmyxtx2r99jjhs04lu980xdyd2ulwwmx9evrfz",
            "entropyxtest:qqfwmv2jm7dsuju9wz27ptdm4e28qh6evfsm66uf2vf4fxmpxfqgym4m2fcyp",
            "entropyxtest:qpcerqk4ltxtyprv9096wrlzjx5mnrlw4fqce6hnl3axy7tkvyjxypjc5dyqs",
            "entropyxtest:qr9m4h44ghmyz4wagktx8kgmh9zj8h8q0f6tc87wuad5xvzkdlwd6uu9plg2c",
            "entropyxtest:qrkxylqkyjtkjr5zs4z5wjmhmj756e84pa05amcw3zn8wdqjvn4tcc2gcqhrw",
            "entropyxtest:qp3w5h9hp9ude4vjpllsm4qpe8rcc5dmeealkl0cnxlgtj4ly7rczqxcdamvr",
            "entropyxtest:qpqen78dezzj4w7rae4n6kvahlr6wft7jy3lcul78709asxksgxc2kr9fgv6j",
            "entropyxtest:qq7upgj3g8klaylc4etwhlmr70t24wu4n4qrlayuw44yd8wx40seje27ah2x7",
            "entropyxtest:qqt2jzgzwy04j8np6ne4g0akmq4gj3fha0gqupr2mjj95u5utzxqvv33mzpcu",
            "entropyxtest:qpcnt3vscphae5q8h576xkufhtuqvntg0ves8jnthgfaxy8ajek8zz3jcg4de",
            "entropyxtest:qz7wzgzvnadgp6v4u6ua9f3hltaa3cv8635mvzlepa63ttt72c6m208g48q0p",
            "entropyxtest:qpqtsd4flc0n4g720mjwk67tnc46xv9ns5xs2khyvlvszy584ej4xq9adw9h9",
            "entropyxtest:qq4uy92hzh9eauypps060g2k7zv2xv9fsgc5gxkwgsvlhc7tw4a3gk5rnpc0k",
            "entropyxtest:qqgfhd3ur2v2xcf35jggre97ar3awl0h62qlmmaaq28dfrhwzgjnxntdugycr",
            "entropyxtest:qzuflj6tgzwjujsym9ap6dvqz9zfwnmkta68fjulax09clh8l4rfslj9j9nnt",
            "entropyxtest:qz6645a8rrf0hmrdvyr9uj673lrr9zwhjvvrytqpjsjdet23czvc784e84lfe",
            "entropyxtest:qz2fvhmk996rmmg44ht0s79gnw647ehu8ncmpf3sf6txhkfmuzuxssceg9sw0",
            "entropyxtest:qr9aflwylzdu99z2z25lzljyeszhs7j02zhfdazydgahq2vg6x8w7nfp3juqq",
            "entropyxtest:qzen7nh0lmzvujlye5sv3nwgwdyew2zp9nz5we7pay65wrt6kfxd6khwja56q",
            "entropyxtest:qq74jrja2mh3wn6853g8ywpfy9nlg0uuzchvpa0cmnvds4tfnpjj5tqgnqm4f",
        ]
    }
    ...
    fn gen0_change_addresses() -> Vec<&'static str> {
        vec![
            "entropyxtest:qrc0xjaq00fq8qzvrudfuk9msag7whnd72nefwq5d07ks4j4d97kzm0x3ertv",
            "entropyxtest:qpf00utzmaa2u8w9353ssuazsv7fzs605eg00l9luyvcwzwj9cx0z4m8n9p5j",
            "entropyxtest:qrkxek2q6eze7lhg8tq0qw9h890lujvjhtnn5vllrkgj2rgudl6xv3ut9j5mu",
            "entropyxtest:qrn0ga4lddypp9w8eygt9vwk92lagr55e2eqjgkfr09az90632jc6namw09ll",
            "entropyxtest:qzga696vavxtrg0heunvlta5ghjucptll9cfs5x0m2j05s55vtl36uhpauwuk",
            "entropyxtest:qq8ernhu26fgt3ap73jalhzl5u5zuergm9f0dcsa8uy7lmcx875hwl3r894fp",
            "entropyxtest:qrauma73jdn0yfwspr7yf39recvjkk3uy5e4309vjc82qq7sxtskjphgwu0sx",
            "entropyxtest:qzk7yd3ep4def7sv7yhl8m0mr7p75zclycrv0x0jfm0gmwte23k0u5f9dclzy",
            "entropyxtest:qzvm7mnhpkrw52c4p85xd5scrpddxnagzmhmz4v8yt6nawwzgjtavu84ft88x",
            "entropyxtest:qq4feppacdug6p6zk2xf4rw400ps92c9h78gctfcdlucvzzjwzyz7j650nw52",
            "entropyxtest:qryepg9agerq4wdzpv39xxjdytktga53dphvs6r4fdjc0gfyndhk7ytpnl5tv",
            "entropyxtest:qpywh5galz3dd3ndkx96ckpvvf5g8t4adaf0k58y4kgf8w06jt5myjrpluvk6",
            "entropyxtest:qq32grys34737mfe5ud5j2v03cjefynuym27q7jsdt28qy72ucv3sv0teqwvm",
            "entropyxtest:qper47ahktzf9lv67a5e9rmfk35pq4xneufhu97px6tlzd0d4qkaklx7m3f7w",
            "entropyxtest:qqal0t8w2y65a4lm5j5y4maxyy4nuwxj6u364eppj5qpxz9s4l7tknfw0u6r3",
            "entropyxtest:qr7p66q7lmdqcf2vnyus38efx3l4apvqvv5sff66n808mtclef2w7vxh3afnn",
            "entropyxtest:qqx4xydd58qe5csedz3l3q7v02e49rwqnydc425d6jchv02el2gdv4055vh0y",
            "entropyxtest:qzyc9l5azcae7y3yltgnl5k2dzzvngp90a0glsepq0dnz8dvp4jyveezpqse8",
            "entropyxtest:qq705x6hl9qdvr03n0t65esevpvzkkt2xj0faxp6luvd2hk2gr76chxw8xhy5",
            "entropyxtest:qzufchm3cy2ej6f4cjpxpnt3g7c2gn77c320qhrnrjqqskpn7vnzsaxg6z0kd",
        ]
    }
