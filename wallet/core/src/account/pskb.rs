...
use futures::stream;
use entropyx_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use entropyx_consensus_client::UtxoEntry as ClientUTXO;
use entropyx_consensus_core::hashing::sighash::{calc_schnorr_signature_hash, SigHashReusedValues};
use entropyx_consensus_core::tx::VerifiableTransaction;
use entropyx_consensus_core::tx::{TransactionInput, UtxoEntry};
use entropyx_txscript::extract_script_pub_key_address;
use entropyx_txscript::opcodes::codes::OpData65;
use entropyx_txscript::script_builder::ScriptBuilder;
use entropyx_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use entropyx_wallet_pskt::bundle::Bundle;
use entropyx_wallet_pskt::prelude::KeySource;
use entropyx_wallet_pskt::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use entropyx_wallet_pskt::pskt::{Creator, PSKT};
...
pub fn pskt_to_pending_transaction(
    ...
    let output: Vec<entropyx_consensus_core::tx::TransactionOutput> = signed_tx.outputs.clone();
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        entropyx_wallet_core::tx::DataKind::Final,
    )?;
