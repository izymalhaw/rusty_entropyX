//!
//! API trait for interfacing with the EntropyX wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `entropyx`.
    ///
    ...
    /// applications such as KDX and entropyxnet.io web wallet using a deprecated derivation path
