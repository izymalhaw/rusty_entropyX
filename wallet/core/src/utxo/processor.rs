...
use entropyx_wrpc_client::EntropyXRpcClient;
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<EntropyXRpcClient>> {
        self.rpc_api().clone().downcast_arc::<EntropyXRpcClient>().ok()
    }
    ...
    pub fn get_current_burn_fee(&self) -> u64 {
        self.network_params().unwrap().get_current_burn_fee()
    }
    ...
    pub async fn handle_daa_score_change(&self, current_daa_score: u64) -> Result<()> {
        ...
        // ENX-CHANGE-BURN: Update the current burn fee
        self.network_params()?.update_current_burn_fee(current_daa_score);
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        log_trace!("Connected to entropyx: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");
