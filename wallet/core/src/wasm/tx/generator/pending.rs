...
use entropyx_consensus_client::{numeric, string};
use entropyx_consensus_client::{Transaction, TransactionT};
...
use entropyx_wrpc_wasm::RpcClient;
    ...
    pub fn sign(&self, js_value: PrivateKeyArrayT, check_fully_signed: Option<bool>) -> Result<()> {
        if let Ok(keys) = js_value.dyn_into::<Array>() {
            let keys = keys
                ...
                .collect::<std::result::Result<Vec<_>, entropyx_wallet_keys::error::Error>>()?;
