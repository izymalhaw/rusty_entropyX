...
use entropyx_consensus_client::UtxoEntry;
use entropyx_consensus_core::constants::UNACCEPTED_DAA_SCORE;
use entropyx_consensus_core::mass::Kip9Version;
use entropyx_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
use entropyx_consensus_core::tx::{Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
...
impl Generator {
    ...
    pub fn try_new(settings: GeneratorSettings, signer: Option<Arc<dyn SignerT>>, abortable: Option<&Abortable>) -> Result<Self> {
        ...
        let (final_transaction_outputs, final_transaction_amount) = match final_transaction_destination {
            PaymentDestination::Change | PaymentDestination::Receive => { // ENX-CHANGE-BURN: add support for receive address
    ...
    fn calc_relay_transaction_compute_fees(&self, data: &Data) -> u64 {
        let base_fee = self.inner.mass_calculator.calc_minimum_transaction_fee_from_mass(self.calc_relay_transaction_mass(data));
        // ENX-CHANGE-BURN: Add the burn fee only for non-sweep transactions
        if self.inner.final_transaction.is_some() {
            base_fee + self.inner.network_params.get_current_burn_fee()
        } else {
            base_fee
        }
    }
    ...
    */
    ...
    fn try_finish_standard_stage_processing(
        ...
        // ENX-CHANGE-BURN: calculate the total amount needed including the burn fee
        let burn_fee = if self.inner.final_transaction.is_some() {
            self.inner.network_params.get_current_burn_fee()
        } else {
            0 // No burn fee for sweep transactions
        };
        let total_required = final_transaction.value_no_fees + transaction_fees + burn_fee;

        // ensure there is enough input value
        if data.aggregate_input_value < total_required {
            // continue aggregating more UTXOs
            return Ok(None);
        }

        let total_stage_value_needed = if self.inner.final_transaction_priority_fee.sender_pays() {
            // ENX-CHANGE-BURN: Add the burn fee to the total stage value needed
            final_transaction.value_with_priority_fee + stage.aggregate_fees + transaction_fees + burn_fee
        } else {
        ...
        if reject {
            ...
            let (mut transaction_fees, change_output_value) = match self.inner.final_transaction_priority_fee {
                Fees::SenderPays(priority_fees) => {
                    ...
                    // ENX-CHANGE-BURN: The transaction fees = relay fees + priority fees + burn fee
                    // First calculate the base fees
                    let base_fees = transaction_fees.checked_add(priority_fees).unwrap();
                    // Then calculate the total fees
                    let total_fees = base_fees.checked_add(burn_fee).unwrap();
                    // Validate the total fees
                    let total_amount_needed = final_transaction.value_no_fees.saturating_add(total_fees);
                    if data.aggregate_input_value < total_amount_needed {
                        return Err(Error::InsufficientFunds {
                            additional_needed: total_amount_needed,
                            origin: "fee_calculation"
                        });
                    }
                    ...
                    (total_fees, change_output_value)
                }
    ...
    pub fn summary(&self) -> GeneratorSummary {
        ...
        // ENX-CHANGE-BURN: Set the burn fee for the transaction
        let burn_fee = if self.inner.final_transaction.is_some() {
            self.inner.network_params.get_current_burn_fee()
        } else {
            0
        ...
        GeneratorSummary {
            ...
            burn_fee: burn_fee, // ENX-CHANGE-BURN:
        }
    }
}
