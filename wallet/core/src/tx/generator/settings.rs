...
impl GeneratorSettings {
    ...
    /// ENX-CHANGE-BURN:
    /// Create generator settings for sweeping UTXOs from a specific address
    pub fn try_new_for_sweep(
        account: Arc<dyn Account>,
        ...
        let filtered_utxos = account
            .utxo_context()
            .context()
            .mature
            .iter()
            .filter(|utxo| utxo.address().as_ref() == Some(&target_address))
            .cloned()
            .collect::<Vec<_>>();
        
        let utxo_iterator = Box::new(filtered_utxos.into_iter());
        ...
        let settings = GeneratorSettings {
            ...
            final_transaction_priority_fee: final_priority_fee,
            final_transaction_destination: final_transaction_destination,
