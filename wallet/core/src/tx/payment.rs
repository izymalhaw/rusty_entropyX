...
use crate::imports::*;
use entropyx_consensus_client::{TransactionOutput, TransactionOutputInner};
...
export interface IPaymentOutput {
     ...
     * you are transacting on (e.g. `entropyx:` for mainnet, `entropyxtest:` for testnet, etc).
     */
...
pub enum PaymentDestination {
    Change,
    Receive, // ENX-CHANGE-BURN: add support for receive address
...
impl PaymentDestination {
    pub fn amount(&self) -> Option<u64> {
        match self {
            Self::Change => None,
            Self::Receive => None, // ENX-CHANGE-BURN: add support for receive address
