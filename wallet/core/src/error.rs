...
use entropyx_bip32::Error as BIP32Error;
use entropyx_consensus_core::sign::Error as CoreSignError;
use entropyx_rpc_core::RpcError as EntropyXRpcError;
use entropyx_wrpc_client::error::Error as EntropyXWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] entropyx_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    EntropyXRpcClientResult(#[from] EntropyXRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    EntropyXWorkflowRpcError(#[from] EntropyXWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] entropyx_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] entropyx_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] entropyx_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] entropyx_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] entropyx_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] entropyx_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] entropyx_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] entropyx_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] entropyx_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] entropyx_wallet_pskt::error::Error),
