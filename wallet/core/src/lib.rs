...
//! the `entropyx` module includes `websocket` module dependency simulating
...
extern crate alloc;
extern crate self as entropyx_wallet_core;
...
pub fn version_with_git_hash() -> String {
    entropyx_utils::git::with_short_hash(env!("CARGO_PKG_VERSION")).to_string()
}
