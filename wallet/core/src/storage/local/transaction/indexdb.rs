...
const ENXRYPTION_KIND: EncryptionKind = EncryptionKind::XChaCha20Poly1305;
                    ...
                    loop {
                        ...
                        if let Ok(record) = transaction_record_from_js_value(&js_value, None) {
                            if record.unixtime_msec.is_some() {
                                let new_js_value = transaction_record_to_js_value(&record, None, ENXRYPTION_KIND)?;
...
impl TransactionRecordStore for TransactionStore {
    ...
    async fn store(&self, transaction_records: &[&TransactionRecord]) -> Result<()> {
        ...
        let items = transaction_records
            ...
            .map(|transaction_record| {
                ...
                let js_value = transaction_record_to_js_value(transaction_record, None, ENXRYPTION_KIND)?;
    ...
    async fn store_transaction_note(
        ...
        call_async_no_send!(async move {
            ...
            let new_js_value = transaction_record_to_js_value(&transaction_record, None, ENXRYPTION_KIND)?;
