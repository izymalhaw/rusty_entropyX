...
/// Source entropyx for a BIP39 mnemonic phrase
pub type EntropyX32 = [u8; KEY_SIZE];
pub type EntropyX16 = [u8; 16];
...
pub struct Mnemonic {
    ...
    /// Source entropyx for this phrase
    entropyx: Vec<u8>,
...
impl Mnemonic {
    ...
    #[wasm_bindgen(getter, js_name = entropyx)]
    pub fn get_entropyx(&self) -> String {
        self.entropyx.to_hex()
    }

    #[wasm_bindgen(setter, js_name = entropyx)]
    pub fn set_entropyx(&mut self, entropyx: String) {
        let vec = Vec::<u8>::from_hex(&entropyx).unwrap_or_else(|err| panic!("invalid entropyx `{entropyx}`: {err}"));
        let len = vec.len();
        if len != 16 && len != 32 {
            panic!("Invalid entropyx: `{entropyx}`")
        }
        self.entropyx = vec;
    }
...
impl Mnemonic {
    ...
    pub fn random_impl(word_count: WordCount, mut rng: impl RngCore + CryptoRng, language: Language) -> Result<Self> {
        match word_count {
            WordCount::Words24 => {
                let mut entropyx = EntropyX32::default();
                rng.fill_bytes(&mut entropyx);
                Self::from_entropyx(entropyx.to_vec(), language)
            }
            WordCount::Words12 => {
                let mut entropyx = EntropyX16::default();
                rng.fill_bytes(&mut entropyx);
                Self::from_entropyx(entropyx.to_vec(), language)
            }
        }
    ...
    /// Create a new BIP39 mnemonic phrase from the given entropyx
    pub fn from_entropyx(entropyx: Vec<u8>, language: Language) -> Result<Self> {
        if entropyx.len() != 16 && entropyx.len() != 32 {
            return Err(Error::String("EntropyX length should be 16 or 32.".to_string()));
        }

        let wordlist = language.wordlist();
        let entropyx = Zeroizing::new(entropyx);
        let checksum_byte = Self::build_checksum(&entropyx)?;

        // First, create a byte iterator for the given entropyx and the first byte of the
        // hash of the entropyx that will serve as the checksum (up to 8 bits for biggest
        // entropyx source).
        //
        ...
        //
        // Given the entropyx is of correct size, this ought to give us the correct word
        // count.
        let phrase = entropyx.iter().chain(Some(&checksum_byte)).bits().map(|bits| wordlist.get_word(bits)).join(" ");

        Ok(Self { language, entropyx: entropyx.to_vec(), phrase })
    }
        ...
        S: AsRef<str>,
    {
        ...
        let mut entropyx = Zeroizing::new(bits.into_bytes());

        let key_size = match entropyx.len() {
            17 => 16,
            33 => 32,
            _ => {
        ...
        let actual_checksum = entropyx[key_size];

        // Truncate to get rid of the byte containing the checksum
        entropyx.truncate(key_size);

        let expected_checksum = Self::build_checksum(&entropyx)?;
    ...
    fn build_checksum(entropyx: &Zeroizing<Vec<u8>>) -> Result<u8> {
        let binding = Sha256::digest(entropyx);
        let bytes = binding.as_slice();
        //println!("len: {}, bytes: {:?}", entropyx.len(), bytes);
        match entropyx.len() {
            16 => {
    ...
    /// Get source entropyx for this phrase.
    pub fn entropyx(&self) -> &Vec<u8> {
        &self.entropyx
    }
