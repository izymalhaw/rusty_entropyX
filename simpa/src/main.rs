...
use entropyx_database::prelude::ConnBuilder;
use entropyx_database::{create_temp_db, load_existing_db};
use entropyx_hashes::Hash;
use entropyx_perf_monitor::{builder::Builder, counters::CountersSnapshot};
use entropyx_utils::fd_budget;
use simulator::network::EntropyXNetworkSimulator;
use std::{collections::VecDeque, sync::Arc, time::Duration};

pub mod simulator;

/// EntropyX Network Simulator
#[derive(Parser, Debug)]
...
fn main() {
    ...
    cfg_if::cfg_if! {
        if #[cfg(feature = "semaphore-trace")] {
            entropyx_core::log::init_logger(None, &format!("{},{}=debug", args.log_level, entropyx_utils::sync::semaphore_module_path()));
        } else {
            entropyx_core::log::init_logger(None, &args.log_level);
        }
    };
...
fn main_impl(mut args: Args) {
    ...
    let stop_perf_monitor = args.perf_metrics.then(|| {
        ...
        let cb = move |counters: CountersSnapshot| {
            trace!("[{}] {}", entropyx_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            trace!("[{}] {}", entropyx_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = EntropyXNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
...
fn apply_args_to_consensus_params(args: &Args, params: &mut Params) {
    ...
    if args.testnet11 {
        info!(
            "Using entropyx-testnet-11 configuration (GHOSTDAG K={}, DAA window size={}, Median time window size={})",
            params.ghostdag_k,
...
mod tests {
    ...
    fn test_pruning_via_simpa() {
        ...
        entropyx_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        entropyx_core::panic::configure_panic();
        main_impl(args);
    }
