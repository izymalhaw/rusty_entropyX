...
use entropyx_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use entropyx_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use entropyx_core::{info, trace};
use entropyx_database::prelude::{StoreError, StoreResult, DB};
use entropyx_hashes::Hash;
use entropyx_index_core::indexed_utxos::BalanceByScriptPublicKey;
use entropyx_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-entropyx), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<entropyx_consensus_core::tx::TransactionOutpoint>> {
