...
mod tests {
    ...
    use entropyx_consensus_core::{
        ...
        coinbase::MinerData,
        constants::{MAX_TX_IN_SEQUENXE_NUM, SOMPI_PER_ENX, TX_VERSION},
    ...
    use entropyx_mining_errors::mempool::RuleResult;
    ...
    fn test_replace_by_fee_in_mempool() {
        ...
        impl TxOp {
            fn change(&self) -> Option<u64> {
                self.change.then_some(900 * SOMPI_PER_ENX)
            }
        }
    ...
    fn test_revalidate_high_priority_transactions() {
        ...
        let (parent_tx, child_tx_1) = create_parent_and_children_transactions(&consensus, vec![3000 * SOMPI_PER_ENX]);
    ...
    fn generate_new_coinbase(address_prefix: Prefix, op: OpType) -> MinerData {
        match op {
            OpType::Usual => get_miner_data(address_prefix), // TODO: use lib_entropyx_wallet.CreateKeyPair, util.NewAddressPublicKeyECDSA equivalents
            OpType::Edcsa => get_miner_data(address_prefix), // TODO: use lib_entropyx_wallet.CreateKeyPair, util.NewAddressPublicKey equivalents
            OpType::True => {
    ...
    fn create_transaction_with_utxo_entry(i: u32, block_daa_score: u64) -> MutableTransaction {
        ...
        let input = TransactionInput::new(previous_outpoint, signature_script, MAX_TX_IN_SEQUENXE_NUM, 1);
        let entry = UtxoEntry::new(SOMPI_PER_ENX, script_public_key.clone(), block_daa_score, true);
        let output = TransactionOutput::new(SOMPI_PER_ENX - DEFAULT_MINIMUM_RELAY_TRANSACTION_FEE, script_public_key);
    ...
    fn create_and_add_funding_transactions(consensus: &Arc<ConsensusMock>, count: usize) -> Vec<Transaction> {
        ...
        (0..count)
            .map(|i| {
                let funding_tx = create_transaction_without_input(vec![1_000 * SOMPI_PER_ENX, 2_500 * SOMPI_PER_ENX + i as u64]);
    ...
    fn create_arrays_of_parent_and_children_transactions(
        ...
        (0..count)
            .map(|i| {
                create_parent_and_children_transactions(consensus, vec![500 * SOMPI_PER_ENX, 3_000 * SOMPI_PER_ENX + i as u64])
            })
            .unzip()
    }
    ...
    fn create_child_and_parent_txs_and_add_parent_to_consensus(consensus: &Arc<ConsensusMock>) -> Transaction {
        let parent_tx = create_transaction_without_input(vec![500 * SOMPI_PER_ENX]);
