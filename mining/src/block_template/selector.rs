use entropyx_core::{time::Stopwatch, trace};
use rand::Rng;
...
mod tests {
    ...
    use entropyx_txscript::{pay_to_script_hash_signature_script, test_helpers::op_true_script};
    ...
    fn test_reject_transaction() {
        ...
        let transactions = (0..TX_INITIAL_COUNT).map(|i| create_transaction(SOMPI_PER_ENX * (i + 1) as u64)).collect_vec();
