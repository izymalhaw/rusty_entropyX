...
use entropyx_txscript::{get_sig_op_count, is_unspendable, script_class::ScriptClass};
...
impl Mempool {
    pub(crate) fn check_transaction_standard_in_isolation(&self, transaction: &MutableTransaction) -> NonStandardResult<()> {
        ...
        // However, in a later version of EntropyX the consensus-valid transaction version range might diverge from the
...
mod tests {
    ...
    fn test_check_transaction_standard_in_isolation() {
        ...
        let dummy_tx_input = TransactionInput::new(dummy_prev_out, dummy_sig_script, MAX_TX_IN_SEQUENXE_NUM, 1);
        let addr_hash = vec![1u8; 32];

        let addr = Address::new(Prefix::Testnet, Version::PubKey, &addr_hash);
        let dummy_script_public_key = entropyx_txscript::pay_to_address_script(&addr);
