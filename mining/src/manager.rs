...
use entropyx_consensusmanager::{spawn_blocking, ConsensusProxy};
use entropyx_core::{debug, error, info, time::Stopwatch, warn};
use entropyx_mining_errors::{manager::MiningManagerError, mempool::RuleError};
...
impl MiningManager {
                ...
                Err(BuilderError::ConsensusError(BlockRuleError::InvalidTransactionsInNewBlock(invalid_transactions))) => {
                    ...
                    invalid_transactions.iter().for_each(|(x, err)| {
                        ...
                        if let Err(err) = removal_result {
                            ...
                            // https://github.com/entropyxnet/entropyx/issues/1553
    ...
    pub(crate) fn get_realtime_feerate_estimations_verbose(
        ...
        prefix: entropyx_addresses::Prefix,
        ...
        {
            let script_public_key = entropyx_txscript::pay_to_address_script(&entropyx_addresses::Address::new(
                prefix,
                entropyx_addresses::Version::PubKey,
                &[0u8; 32],
            ));
            let miner_data: MinerData = MinerData::new(script_public_key, vec![]);

            let BlockTemplate { block: entropyx_consensus_core::block::MutableBlock { transactions, .. }, calculated_fees, .. } =
...
mod tests {
    use super::*;
    use entropyx_consensus_core::subnets;
    use std::iter::repeat;
