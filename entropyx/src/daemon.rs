...
use entropyx_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use entropyx_core::{core::Core, debug, info};
use entropyx_core::{entropyx_env::version, task::tick::TickService};
use entropyx_database::prelude::CachePolicy;
use entropyx_grpc_server::service::GrpcService;
use entropyx_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use entropyx_rpc_service::service::RpcCoreService;
use entropyx_txscript::caches::TxScriptCacheCounters;
use entropyx_utils::git;
use entropyx_utils::networking::ContextualNetAddress;
use entropyx_utils::sysinfo::SystemInfo;
use entropyx_utils_tower::counters::TowerConnectionCounters;

use entropyx_addressmanager::AddressManager;
use entropyx_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
use entropyx_consensus::{
...
use entropyx_p2p_flows::{flow_context::FlowContext, service::P2pService};

use entropyx_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use entropyx_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use entropyx_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the entropyx process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the entropyx
/// process. (Rusty EntropyX will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("rusty-entropyx");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".rusty-entropyx");
}
...
/// the application folder that contains entropyx logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                entropyx_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, entropyx_utils::sync::semaphore_module_path()));
            } else {
                entropyx_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the EntropyX command line to confirm all interactive questions)";
    ...
    let mut meta_db = entropyx_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = entropyx_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from entropyx DB version
    ...
    {
        let msg =
            "Node database is from a different EntropyX *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
        get_user_approval_or_exit(msg, args.yes);

        info!("Deleting databases from previous EntropyX version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = entropyx_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", entropyx_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", entropyx_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", entropyx_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = entropyx_database::prelude::ConnBuilder::default()
