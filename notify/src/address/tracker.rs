...
use entropyx_addresses::{Address, Prefix};
use entropyx_consensus_core::tx::ScriptPublicKey;
...
/// Tracker of a set of [`Address`](entropyx_addresses::Address), indexing and counting registrations
///
/// #### Implementation design
///
/// Each [`Address`](entropyx_addresses::Address) is stored internally as a [`ScriptPubKey`](entropyx_consensus_core::tx::ScriptPublicKey).
...
mod tests {
    ...
    fn create_addresses(start: usize, count: usize) -> Vec<Address> {
        (start..start + count)
            .map(|i| Address::new(Prefix::Mainnet, entropyx_addresses::Version::PubKey, &Uint256::from_u64(i as u64).to_le_bytes()))
            .collect()
    }
