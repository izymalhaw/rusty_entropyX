...
use entropyx_consensus_core::api::counters::ProcessingCounters;
use entropyx_consensus_core::errors::block::RuleError;
...
use entropyx_core::{
    core::Core,
    debug,
    entropyx_env::version,
    signals::Shutdown,
    task::service::{AsyncService, AsyncServiceError, AsyncServiceFuture},
    task::tick::TickService,
    trace, warn,
};
use entropyx_index_core::indexed_utxos::BalanceByScriptPublicKey;
use entropyx_index_core::{
...
use entropyx_mining::feerate::FeeEstimateVerbose;
use entropyx_mining::model::tx_query::TransactionQuery;
use entropyx_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use entropyx_notify::listener::ListenerLifespan;
use entropyx_notify::subscription::context::SubscriptionContext;
use entropyx_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use entropyx_notify::{
...
use entropyx_p2p_flows::flow_context::FlowContext;
use entropyx_p2p_lib::common::ProtocolError;
use entropyx_perf_monitor::{counters::CountersSnapshot, Monitor as PerfMonitor};
...
use entropyx_txscript::{extract_script_pub_key_address, pay_to_address_script};
use entropyx_utils::hex::FromHex; // ENX-CHANGE-BURN:
use entropyx_utils::expiring_cache::ExpiringCache;
use entropyx_utils::sysinfo::SystemInfo;
use entropyx_utils::{channel::Channel, triggers::SingleTrigger};
use entropyx_utils_tower::counters::TowerConnectionCounters;
use entropyx_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at entropyx_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<entropyx_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
}
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. If you are on TN11 this is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(entropyx_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }

        // Build block template
        let script_public_key = entropyx_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: entropyx_hashes::Hash = entropyx_hashes::Hash::from_bytes([0xff; entropyx_hashes::HASH_SIZE]);
    ...
    /// ENX-CHANGE-BURN: ApiService
    async fn get_total_burned_amount_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        _request: GetTotalBurnedAmountRequest,
    ) -> RpcResult<GetTotalBurnedAmountResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        match session.get_total_burned_amount()? {
            Some(total) => Ok(GetTotalBurnedAmountResponse { total_burned: total }),
            None => Err(RpcError::General("No burned amount found".into())),
        }
    }

    async fn get_burn_records_by_range_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetBurnRecordsByRangeRequest,
    ) -> RpcResult<GetBurnRecordsByRangeResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        let records = session.get_burn_records_by_range(request.start_daa, request.end_daa)?
            .into_iter()
            .map(|r| RpcBurnRecord {
                block_hash: r.block_hash.to_string(),
                block_daa_score: r.block_daa_score,
                burned_value: r.burned_value,
                total_burned: r.total_burned,
                timestamp: r.timestamp,
            })
            .collect();
            
        Ok(GetBurnRecordsByRangeResponse { records })
    }

    async fn get_burn_record_by_utxo_id_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetBurnRecordByUtxoIdRequest,
    ) -> RpcResult<GetBurnRecordByUtxoIdResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        
        // add check for utxo_id
        if request.utxo_id.is_empty() {
            return Err(RpcError::InvalidParams("Invalid UTXO ID".into()));
        }

        let block_hash = RpcHash::from_hex(request.utxo_id.as_str())?;
        let record = session.get_burn_record(block_hash)?
            .map(|r| RpcBurnRecord {
                block_hash: r.block_hash.to_string(),
                block_daa_score: r.block_daa_score,
                burned_value: r.burned_value,
        ...
        Ok(GetBurnRecordByUtxoIdResponse { record })
    }

    async fn get_current_burn_fee_call(
        &self,
        _connection: Option<&DynRpcConnection>,
        request: GetCurrentBurnFeeRequest,
    ) -> RpcResult<GetCurrentBurnFeeResponse> {
        let session = self.consensus_manager.consensus().unguarded_session();
        let burn_fee = session.get_current_burn_fee(request.current_daa_score)?;
            
        Ok(GetCurrentBurnFeeResponse { burn_fee })
    }
    // ENX-CHANGE-BURN: End
}
