...
use entropyx_consensus_core::block::{Block, MutableBlock};
    ...
    fn try_from(item: RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .into_iter()
                    .map(entropyx_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<entropyx_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}
