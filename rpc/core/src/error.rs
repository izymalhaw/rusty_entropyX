use entropyx_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId};
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] entropyx_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] entropyx_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] entropyx_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] entropyx_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] entropyx_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] entropyx_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] entropyx_txscript::script_class::Error),
