...
use entropyx_consensus_core::api::stats::BlockCount;
use entropyx_core::debug;
use entropyx_notify::subscription::{context::SubscriptionContext, single::UtxosChangedSubscription, Command};
use entropyx_utils::hex::ToHex;
...
///  ENX-CHANGE-BURN:
///  wRPC request for RpcApiOps::GetTotalBurnedAmount request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetTotalBurnedAmountRequest {}

impl GetTotalBurnedAmountRequest {
    pub fn new() -> Self {
        Self {}
    }
}

impl Serializer for GetTotalBurnedAmountRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        Ok(())
    }
...
impl Deserializer for GetTotalBurnedAmountRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        Ok(Self {})
    }
...
///  ENX-CHANGE-BURN:
///  wRPC response for RpcApiOps::GetTotalBurnedAmount request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetTotalBurnedAmountResponse {
    pub total_burned: u64,
}

impl GetTotalBurnedAmountResponse {
    pub fn new(total_burned: u64) -> Self {
        Self { total_burned }
    }
}

impl Serializer for GetTotalBurnedAmountResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(u64, &self.total_burned, writer)?;
        Ok(())
    }
}

impl Deserializer for GetTotalBurnedAmountResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let total_burned = load!(u64, reader)?;
        Ok(Self { total_burned })
    }
}

///  ENX-CHANGE-BURN:
///  wRPC request for RpcApiOps::GetBurnRecordsByRange request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetBurnRecordsByRangeRequest {
    pub start_daa: u64,
    pub end_daa: u64,
}

impl GetBurnRecordsByRangeRequest {
    pub fn new(start_daa: u64, end_daa: u64) -> Self {
        Self { start_daa, end_daa }
    }
}


impl Serializer for GetBurnRecordsByRangeRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(u64, &self.start_daa, writer)?;
        store!(u64, &self.end_daa, writer)?;
        Ok(())
    }
}

impl Deserializer for GetBurnRecordsByRangeRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let start_daa = load!(u64, reader)?;
        let end_daa = load!(u64, reader)?;
        Ok(Self { start_daa, end_daa })
    }
}

///  ENX-CHANGE-BURN:
///  wRPC response for RpcApiOps::GetBurnRecordsByRange request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetBurnRecordsByRangeResponse {
    pub records: Vec<RpcBurnRecord>,
}

impl GetBurnRecordsByRangeResponse {
    pub fn new(records: Vec<RpcBurnRecord>) -> Self {
        Self { records }
    }
}

impl Serializer for GetBurnRecordsByRangeResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        serialize!(Vec<RpcBurnRecord>, &self.records, writer)?;
        Ok(())
    }
}

impl Deserializer for GetBurnRecordsByRangeResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let records = deserialize!(Vec<RpcBurnRecord>, reader)?;
        Ok(Self { records })
    }
}

///  ENX-CHANGE-BURN:
///  wRPC request for RpcApiOps::GetBurnRecordByUtxoId request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetBurnRecordByUtxoIdRequest {
    pub utxo_id: String,
}

impl GetBurnRecordByUtxoIdRequest {
    pub fn new(utxo_id: String) -> Self {
        Self { utxo_id }
    }
}


impl Serializer for GetBurnRecordByUtxoIdRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(String, &self.utxo_id, writer)?;
        Ok(())
    }
}

impl Deserializer for GetBurnRecordByUtxoIdRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let utxo_id = load!(String, reader)?;
        Ok(Self { utxo_id })
    }
}

///  ENX-CHANGE-BURN:
///  wRPC response for RpcApiOps::GetBurnRecordByUtxoId request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetBurnRecordByUtxoIdResponse {
    pub record: Option<RpcBurnRecord>,
}

impl GetBurnRecordByUtxoIdResponse {
    pub fn new(record: Option<RpcBurnRecord>) -> Self {
        Self { record }
    }
}

impl Serializer for GetBurnRecordByUtxoIdResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        serialize!(Option<RpcBurnRecord>, &self.record, writer)?;
        Ok(())
    }
}

impl Deserializer for GetBurnRecordByUtxoIdResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let record = deserialize!(Option<RpcBurnRecord>, reader)?;
        Ok(Self { record })
    }
}
...
impl RpcBurnRecord {
    pub fn new(block_hash: String, block_daa_score: u64, burned_value: u64, total_burned: u64, timestamp: u64) -> Self {
        Self { block_hash, block_daa_score, burned_value, total_burned, timestamp }
    }
}

impl Serializer for RpcBurnRecord {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(String, &self.block_hash, writer)?;
        store!(u64, &self.block_daa_score, writer)?;
        store!(u64, &self.burned_value, writer)?;
        store!(u64, &self.total_burned, writer)?;
...
///  ENX-CHANGE-BURN:
///  wRPC request for RpcApiOps::GetCurrentBurnFee request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetCurrentBurnFeeRequest {
    pub current_daa_score: u64,
}

impl GetCurrentBurnFeeRequest {
    pub fn new(current_daa_score: u64) -> Self {
        Self { current_daa_score }
    }
}

impl Serializer for GetCurrentBurnFeeRequest {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
...
impl Deserializer for GetCurrentBurnFeeRequest {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let current_daa_score = load!(u64, reader)?;
        Ok(Self { current_daa_score })
    }
}


///  ENX-CHANGE-BURN:
///  wRPC response for RpcApiOps::GetCurrentBurnFee request
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct GetCurrentBurnFeeResponse {
    pub burn_fee: u64,
}

impl GetCurrentBurnFeeResponse {
    pub fn new(burn_fee: u64) -> Self {
        Self { burn_fee }
    }
}

impl Serializer for GetCurrentBurnFeeResponse {
    fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
        store!(u16, &1, writer)?;
        store!(u64, &self.burn_fee, writer)?;
        Ok(())
    }
}

impl Deserializer for GetCurrentBurnFeeResponse {
    fn deserialize<R: std::io::Read>(reader: &mut R) -> std::io::Result<Self> {
        let _version = load!(u16, reader)?;
        let burn_fee = load!(u64, reader)?;
        Ok(Self { burn_fee })
    }
}
