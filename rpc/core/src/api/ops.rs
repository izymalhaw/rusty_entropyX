...
pub enum RpcApiOps {
    ...
    GetSyncStatus = 115,
    /// Returns the network this EntropyX is connected to (Mainnet, Testnet)
    ...
    /// Returns a list of all the addresses (IP, port) this EntropyX knows and a list of all addresses that are currently banned by this EntropyX
    ...
    /// Returns a list of the peers currently connected to this EntropyX, along with some statistics on them
    GetConnectedPeerInfo = 123,
    /// Instructs EntropyX to connect to a given IP address.
    AddPeer = 124,
    ...
    /// ENX-CHANGE-BURN: Get total burned amount
    GetTotalBurnedAmount = 150,
    /// ENX-CHANGE-BURN: Get burn records by range
    GetBurnRecordsByRange = 151,
    /// ENX-CHANGE-BURN: Get burn record by utxoId
    GetBurnRecordByUtxoId = 152,
    /// ENX-CHANGE-BURN: Get current burn fee
    GetCurrentBurnFee = 153,
}
