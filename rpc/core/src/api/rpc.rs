...
use entropyx_notify::{listener::ListenerId, scope::Scope, subscription::Command};
use std::sync::Arc;
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known entropyx addresses in the current network (mainnet, testnet, etc.)
    ...
    // ENX-CHANGE-BURN:
    // Query the total amount of burned tokens
    async fn get_total_burned_amount(&self) -> RpcResult<GetTotalBurnedAmountResponse> {
        Ok(self.get_total_burned_amount_call(None, GetTotalBurnedAmountRequest {})
            .await?)
    }
    ...
    async fn get_total_burned_amount_call(
        &self,
        connection: Option<&DynRpcConnection>,
        request: GetTotalBurnedAmountRequest,
    ) -> RpcResult<GetTotalBurnedAmountResponse>;

    // ENX-CHANGE-BURN:
    // Query the burn records by range
    async fn get_burn_records_by_range(
        &self,
        start_daa: u64,
        end_daa: u64,
    ) -> RpcResult<GetBurnRecordsByRangeResponse> {
        Ok(self.get_burn_records_by_range_call(
            None,
            GetBurnRecordsByRangeRequest { start_daa, end_daa }
        ).await?)
    }
    ...
    // ENX-CHANGE-BURN:
    // Query the burn record by utxoId
    async fn get_burn_record_by_utxo_id(
        &self,
        utxo_id: String,
    ) -> RpcResult<GetBurnRecordByUtxoIdResponse> {
        Ok(self.get_burn_record_by_utxo_id_call(
            None,
            GetBurnRecordByUtxoIdRequest { utxo_id }
        ).await?)
    }
    ...
    // ENX-CHANGE-BURN:
    // Query the current burn fee per transaction
    async fn get_current_burn_fee(
        &self,
        current_daa_score: u64,
    ) -> RpcResult<GetCurrentBurnFeeResponse> {
        Ok(self.get_current_burn_fee_call(
            None,
            GetCurrentBurnFeeRequest { current_daa_score }
        ).await?)
    }
