...
use entropyx_addresses::AddressOrStringArrayT;
use entropyx_consensus_client::Transaction;
use entropyx_consensus_client::UtxoEntryReference;
use entropyx_consensus_core::tx as cctx;
use entropyx_rpc_macros::declare_typescript_wasm_interface as declare;
...
declare! {
    IGetTotalBurnedAmountRequest,
    r#"
    /**
     * Get the total amount of ENX burned from the network.
     * 
     * @category Node RPC
     */
    export interface IGetTotalBurnedAmountRequest { }
    "#,
}

try_from! ( args: IGetTotalBurnedAmountRequest, GetTotalBurnedAmountRequest, {
    Ok(from_value(args.into())?)
});

declare! {
    IGetTotalBurnedAmountResponse,
    r#"
    /**
     * Response containing the total amount of ENX burned.
     * 
     * @category Node RPC
     */
    export interface IGetTotalBurnedAmountResponse {
        totalBurned: bigint;
    }
    "#,
}

try_from!( args: GetTotalBurnedAmountResponse, IGetTotalBurnedAmountResponse, {
    Ok(to_value(&args)?.into())
});
...
declare! {
    IGetCurrentBurnFeeRequest,
    r#"
    /**
     * Get the current burn fee required for transactions.
     * 
     * @category Node RPC
     */
    export interface IGetCurrentBurnFeeRequest { }
    "#,
}

try_from! ( args: IGetCurrentBurnFeeRequest, GetCurrentBurnFeeRequest, {
    Ok(from_value(args.into())?)
});

declare! {
    IGetCurrentBurnFeeResponse,
    r#"
    /**
     * Response containing the current burn fee.
     * 
     * @category Node RPC
     */
    export interface IGetCurrentBurnFeeResponse {
        burnFee: bigint;
    }
    "#,
}

try_from!( args: GetCurrentBurnFeeResponse, IGetCurrentBurnFeeResponse, {
    Ok(to_value(&args)?.into())
});
...
declare! {
    IGetBurnRecordsByRangeRequest,
    r#"
    /**
     * Get burn records within a specified block range.
     * 
     * @category Node RPC
     */
    export interface IGetBurnRecordsByRangeRequest {
        startDaa: number;
        endDaa: number;
    }
    "#,
...
try_from! ( args: IGetBurnRecordsByRangeRequest, GetBurnRecordsByRangeRequest, {
    Ok(from_value(args.into())?)
});
...
declare! {
    ...
    r#"
    /**
     * A record of ENX burned in a transaction.
     * 
     * @category Node RPC
     */
...
try_from!( record: RpcBurnRecord, IBurnRecord, {
    let burn_record = IBurnRecord::default();
    burn_record.set("blockHash", &JsValue::from_str(&record.block_hash.to_string()))?;
    burn_record.set("blockDaaScore", &JsValue::from_f64(record.block_daa_score as f64))?;
    burn_record.set("burnedValue", &js_sys::BigInt::from(record.burned_value).into())?;
    burn_record.set("totalBurned", &js_sys::BigInt::from(record.total_burned).into())?;
    burn_record.set("timestamp", &JsValue::from_f64(record.timestamp as f64))?;
    Ok(burn_record)
});

declare! {
    IGetBurnRecordsByRangeResponse,
    r#"
    /**
     * Response containing burn records within the specified range.
     * 
     * @category Node RPC
     */
    export interface IGetBurnRecordsByRangeResponse {
        records: IBurnRecord[];
    }
    "#,
}

try_from!( args: GetBurnRecordsByRangeResponse, IGetBurnRecordsByRangeResponse, {
    Ok(to_value(&args)?.into())
});
...
declare! {
    IGetBurnRecordByUtxoIdRequest,
    r#"
    /**
     * Get burn record for a specific UTXO ID.
     * 
     * @category Node RPC
     */
    export interface IGetBurnRecordByUtxoIdRequest {
        utxoId: string;
    }
    "#,
}

try_from! ( args: IGetBurnRecordByUtxoIdRequest, GetBurnRecordByUtxoIdRequest, {
    Ok(from_value(args.into())?)
});

declare! {
    IGetBurnRecordByUtxoIdResponse,
    r#"
    /**
     * Response containing burn record for the specified UTXO ID.
     * 
     * @category Node RPC
     */
    export interface IGetBurnRecordByUtxoIdResponse {
        record?: IBurnRecord;
    }
    "#,
}

try_from!( args: GetBurnRecordByUtxoIdResponse, IGetBurnRecordByUtxoIdResponse, {
    Ok(to_value(&args)?.into())
});
