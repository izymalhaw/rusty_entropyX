...
struct RpcTable {
    ...
    entropyx_request_type: Expr,
    entropyx_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, EntropyXRequestType, EntropyXResponseType, EntropyXPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let entropyx_request_type = iter.next().unwrap().clone();
        let entropyx_response_type = iter.next().unwrap().clone();
        ...
        Ok(RpcTable {
            ...
            server_ctx_type,
            connection_ctx_type,
            entropyx_request_type,
            entropyx_response_type,
            payload_ops,
            handlers,
        })
    }
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let entropyx_request_type = &self.entropyx_request_type;
        let entropyx_response_type = &self.entropyx_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #entropyx_request_type, #entropyx_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #entropyx_request_type| {
                                Box::pin(async move {
                                    let mut response: #entropyx_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #entropyx_request_type, #entropyx_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #entropyx_request_type| {
                                Box::pin(async move {
                                    let mut response: #entropyx_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match entropyx_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {
