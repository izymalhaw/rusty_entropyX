...
use entropyx_grpc_core::{
    protowire::{
        ...
        EntropyXRequest, EntropyXResponse,
    },
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<EntropyXResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<EntropyXRequest>>,
