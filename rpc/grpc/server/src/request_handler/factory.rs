...
use super::{
    ...
    interface::{Interface, EntropyXMethod, EntropyXRoutingPolicy},
    method::Method,
};
use crate::{
...
use entropyx_grpc_core::protowire::{entropy_x_request::Payload, *};
use entropyx_grpc_core::{ops::EntropyXPayloadOps, protowire::NotifyFinalityConflictResponseMessage};
use entropyx_notify::{scope::FinalityConflictResolvedScope, subscriber::SubscriptionManager};
use entropyx_rpc_core::{SubmitBlockRejectReason, SubmitBlockReport, SubmitBlockResponse};
use entropyx_rpc_macros::build_grpc_server_interface;
...
impl Factory {
    ...
    pub fn new_interface(server_ctx: ServerContext, network_bps: u64) -> Interface {
        ...
        let method: EntropyXMethod = Method::new(|server_ctx: ServerContext, connection: Connection, request: EntropyXRequest| {
            Box::pin(async move {
                let mut response: EntropyXResponse = match request.payload {
                    Some(Payload::NotifyFinalityConflictRequest(ref request)) => {
                        match entropyx_rpc_core::NotifyFinalityConflictRequest::try_from(request) {
                            Ok(request) => {
        ...
        interface.replace_method(EntropyXPayloadOps::NotifyFinalityConflict, method);
        ...
        interface.set_method_properties(
            ...
            EntropyXRoutingPolicy::DropIfFull(Arc::new(Box::new(|_: &EntropyXRequest| {
