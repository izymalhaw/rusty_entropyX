...
use entropyx_notify::scope::{NewBlockTemplateScope, Scope};
use entropyx_rpc_core::api::rpc::RpcApi;
use entropyx_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    entropyx_core::log::try_init_logger("info, entropyx_grpc_core=trace, entropyx_grpc_server=trace, entropyx_grpc_client=trace");
