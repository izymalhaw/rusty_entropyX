use crate::{
    error::{Error, Result},
    resolver::{matcher::Matcher, EntropyXResponseReceiver, EntropyXResponseSender, Resolver},
};
...
struct Pending {
    ...
    sender: EntropyXResponseSender,
}

impl Pending {
    fn new(op: EntropyXPayloadOps, request: EntropyXRequest, sender: EntropyXResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &EntropyXResponse, response_op: EntropyXPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: EntropyXPayloadOps, request: &EntropyXRequest) -> EntropyXResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<EntropyXResponse>>();
        {
    ...
    fn handle_response(&self, response: EntropyXResponse) {
        let response_op: EntropyXPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");
