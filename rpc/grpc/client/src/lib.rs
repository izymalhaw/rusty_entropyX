...
use entropyx_core::{debug, error, trace};
use entropyx_grpc_core::{
    channel::NotificationChannel,
    ops::EntropyXPayloadOps,
    protowire::{entropy_x_request, rpc_client::RpcClient, GetInfoRequestMessage, EntropyXRequest, EntropyXResponse},
...
use entropyx_utils::{channel::Channel, triggers::DuplexTrigger};
use entropyx_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(EntropyXPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // ENX-CHANGE-BURN: Burn API
    route!(get_total_burned_amount_call, GetTotalBurnedAmount);
    route!(get_burn_records_by_range_call, GetBurnRecordsByRange);
    route!(get_burn_record_by_utxo_id_call, GetBurnRecordByUtxoId);
    route!(get_current_burn_fee_call, GetCurrentBurnFee);
...
type EntropyXRequestSender = async_channel::Sender<EntropyXRequest>;
type EntropyXRequestReceiver = async_channel::Receiver<EntropyXRequest>;
...
/// Data flow:
/// ```
/// //   EntropyXRequest -> request_send -> stream -> EntropyXResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: EntropyXRequestSender,
    request_receiver: EntropyXRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<EntropyXResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual EntropyXRequest to EntropyXResponse stream
        let mut stream: Streaming<EntropyXResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: EntropyXPayloadOps, request: impl Into<EntropyXRequest>) -> Result<EntropyXResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: EntropyXRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<EntropyXResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = entropy_x_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = entropy_x_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }
