...
use entropyx_rpc_core::{RpcError, RpcHash};
use std::str::FromStr;
...
from!(item: &entropyx_rpc_core::RpcBlock, protowire::RpcBlock, {
    Self {
...
from!(item: &entropyx_rpc_core::RpcRawBlock, protowire::RpcBlock, {
    Self {
        header: Some(protowire::RpcBlockHeader::from(&item.header)),
        transactions: item.transactions.iter().map(protowire::RpcTransaction::from).collect(),
        verbose_data: None,
...
from!(item: &entropyx_rpc_core::RpcBlockVerboseData, protowire::RpcBlockVerboseData, {
    Self {
...
try_from!(item: &protowire::RpcBlock, entropyx_rpc_core::RpcBlock, {
    Self {
        header: item
            .header
            .as_ref()
            ...
            .try_into()?,
        transactions: item.transactions.iter().map(entropyx_rpc_core::RpcTransaction::try_from).collect::<Result<Vec<_>, _>>()?,
        verbose_data: item.verbose_data.as_ref().map(entropyx_rpc_core::RpcBlockVerboseData::try_from).transpose()?,
    }
});

try_from!(item: &protowire::RpcBlock, entropyx_rpc_core::RpcRawBlock, {
    Self {
        header: item
            .header
            .as_ref()
...
try_from!(item: &protowire::RpcBlockVerboseData, entropyx_rpc_core::RpcBlockVerboseData, {
    Self {
        ...
        transaction_ids: item
            ...
            .collect::<Result<Vec<entropyx_rpc_core::RpcHash>, faster_hex::Error>>()?,
        is_header_only: item.is_header_only,
        blue_score: item.blue_score,
