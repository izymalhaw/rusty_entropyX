...
use entropyx_rpc_core::{FromRpcHex, RpcError, RpcHash, RpcResult, ToRpcHex};
use std::str::FromStr;
...
from!(item: &entropyx_rpc_core::RpcHeader, protowire::RpcBlockHeader, {
    Self {
...
from!(item: &entropyx_rpc_core::RpcRawHeader, protowire::RpcBlockHeader, {
    Self {
        version: item.version.into(),
        parents: item.parents_by_level.iter().map(protowire::RpcBlockLevelParents::from).collect(),
        hash_merkle_root: item.hash_merkle_root.to_string(),
...
try_from!(item: &protowire::RpcBlockHeader, entropyx_rpc_core::RpcHeader, {
    ...
    let header = Header::new_finalized(
        ...
        item.nonce,
        item.daa_score,
        entropyx_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        item.blue_score,
...
try_from!(item: &protowire::RpcBlockHeader, entropyx_rpc_core::RpcRawHeader, {
    Self {
        ...
        nonce: item.nonce,
        daa_score: item.daa_score,
        blue_work: entropyx_rpc_core::RpcBlueWorkType::from_rpc_hex(&item.blue_work)?,
        blue_score: item.blue_score,
...
mod tests {
    use crate::protowire;
    use entropyx_consensus_core::{block::Block, header::Header};
    use entropyx_rpc_core::{RpcBlock, RpcHash, RpcHeader};
