...
pub use entropyx_rpc_macros::build_wrpc_client_interface;
use std::fmt::Debug;
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("EntropyX wRPC::{encoding} connecting to: {url}");
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: Serializable<entropyx_rpc_core::Notification>| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: EntropyX RPC notification is not consumed by user: {:?}", notification.into_inner());
                        }
                        Ok(())
                    })
                }),
...
impl Debug for Inner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EntropyXRpcClient")
            .field("rpc", &"rpc")
...
/// [`EntropyXRpcClient`] allows connection to the EntropyX wRPC Server via
...
///
#[derive(Clone)]
pub struct EntropyXRpcClient {
    inner: Arc<Inner>,
}

impl Debug for EntropyXRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("EntropyXRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl EntropyXRpcClient {
    /// Create a new `EntropyXRpcClient` with the given Encoding, and an optional url or a Resolver.
    /// Please note that if you pass the url to the constructor, it will force the EntropyXRpcClient
    ...
    ) -> Result<EntropyXRpcClient> {
        Self::new_with_args(encoding, url, resolver, network_id, subscription_context)
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<EntropyXRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = EntropyXRpcClient { inner };
        ...
        // let client = EntropyXRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(EntropyXRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(EntropyXRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(EntropyXRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
...
#[async_trait]
impl RpcApi for EntropyXRpcClient {
    //
    ...
    build_wrpc_client_interface!(
        ...
        [
            ...
            Unban,
            GetTotalBurnedAmount, // ENX-CHANGE-BURN:
            GetBurnRecordsByRange, // ENX-CHANGE-BURN:
            GetBurnRecordByUtxoId, // ENX-CHANGE-BURN:
            GetCurrentBurnFee, // ENX-CHANGE-BURN:
        ]
    );
