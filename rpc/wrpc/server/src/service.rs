...
impl Default for Options {
    fn default() -> Self {
        Options { listen_address: "0.0.0.0:17110".to_owned(), verbose: false, grpc_proxy_address: None }
    }
}

/// ### EntropyXRpcHandler
///
/// [`EntropyXRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct EntropyXRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl EntropyXRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        EntropyXRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for EntropyXRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "entropyx" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<EntropyXRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(EntropyXRpcHandler::new(tasks, *encoding, core_service, options.clone()));
