// Example of simple client to connect with EntropyX node using wRPC connection and collect some node and network basic data

use entropyx_rpc_core::{api::rpc::RpcApi, GetBlockDagInfoResponse, GetServerInfoResponse};
use entropyx_wrpc_client::{
    client::{ConnectOptions, ConnectStrategy},
    prelude::NetworkId,
    prelude::NetworkType,
    result::Result,
    EntropyXRpcClient, Resolver, WrpcEncoding,
};
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to EntropyX node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // Verify your EntropyX node is runnning with --rpclisten-borsh=0.0.0.0:17110 parameter
    ...
    // Define the network your EntropyX node is connected to
    ...
    let client = EntropyXRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected EntropyX node
    client.connect(Some(options)).await?;

    // Retrieve and show EntropyX node information
    ...
    // Retrieve and show EntropyX network information
    ...
    // Disconnect client from EntropyX node
