// Basic example of a EntropyX wRPC client that connects to a node
...
// EntropyX RPC primitives
use entropyx_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use entropyx_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic EntropyX RPC client instance using Borsh encoding.
        let client = Arc::new(EntropyXRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
    ...
    async fn start_event_task(&self) -> Result<()> {
        ...
        // in the rusty entropyx framework)
