...
(async () => {
    ...
    console.log("Virtual DAA score:", virtualDaaScore);

    let ret = await rpc.getBalanceByAddress({ address : sourceAddress.toString() });
    console.log(`Source Address Balance: ${sompiToEntropyXString(ret.balance)}`);

    let { entries } = (await rpc.getUtxosByAddresses([sourceAddress.toString()]));
    ...
    if (!entries.length) {
        ...
        let { transactions, summary } = await createTransactions({
            entries: [entries[0]],
            outputs: [{ address : destinationAddress, amount : entropyxToSompi("0.0001")}],
            priorityFee: 0n,
